...
\section{Vectores}
En \cite[p.~5]{marsden_calculo_2013} se define ``vector como un segmento de recta dirigido que comienza en el origen,
...
El ángulo $\theta $ entre los vectores $\ve{u}$ y $\ve{v}$ satisface la relación
...
Además $\left\Vert \ve{u}\times \ve{v}\right\Vert =\left\Vert \ve{u}\right\Vert\left\Vert \ve{v}\right\Vert\sin\theta;$ estos resultados se pueden consultar en detalle en \textcite{apostol_calculus_1999}.

La siguiente es una conocida forma de evaluar este producto:
$$\ve{u}\times \ve{v}=\left|
\begin{array}{ccc} \vi & \vj & \vk \\ a_1 &a_2&a_3 \\b_1 &b_2&b_3 \\
\end{array} \right|  $$
\textls[-20]{El producto cruz es útil en el desarrollo de algunos de los siguientes resultados.}
\subsection{Proyección ortogonal}
%
La proyección ortogonal del vector $\ve{v}$ sobre el vector
...
El valor $\left| k\right| $ será positivo si
${\rm Proy}_{\ve{u}}\ve{v}$ posee el mismo sentido de $\ve{u}$, de lo
contrario será negativo. Al igualar las expresiones para $\cos \theta $ y considerando el caso $k>0$,
...
En el gráfico la línea discontinua corresponde al vector
...
\begin{figure}
  ...
  \includegraphics[height=3.6cm]{Fig/2.pdf} \hspace{5mm}
  \includegraphics[height=3.6cm]{Fig/3.pdf}
  \fuentepropia
\end{figure}
...
\end{equation}
\textls[-10]{El conjunto de puntos para los cuales la función $f$ está definida se denomina \emph{dominio} de $f$ y el conjunto de puntos de $\mathbb{R}^m$ que son imagen de algún punto~del dominio se denomina \emph{rango} de $f$. De acuerdo con \cite{apostol_calculus_1999}, a las funciones definidas cuando $n=m=1$ se denominan \emph{funciones reales}, las cuales se estudian en los cursos de \emph{cálculo diferencial} y \emph{cálculo integral}. Si $n>1$ y $m=1$ obtendremos \emph{campos escalares}; \index{Campos escalares} \index{Campos vectoriales} llamaremos \emph{funciones vectoriales} a las obtenidas cuando $n=1$ y $m>1$; mientras que los \emph{campos vectoriales} son funciones obtenidas cuando $n>1$ y $m>1$. \index{Funciones vectoriales}}
...
\begin{itemize}
  \item $\overrightarrow{r}(t) =\left\langle \ln t,\frac{1}{t},t^{2}\right\rangle $, es
    una función vectorial en $\mathbb{R}^{3}$ con dominio $(0,\infty )$.
...
En cuanto al dominio de la función $f(x,y) =\frac{x^{4}-y^{4}}{x^{2}+y^{2}}$,
...
\item Usando la definición, demostrar que $\lim\limits_{(x,y) \rightarrow \left( 1,1\right) }\left\langle
  ...
  $\delta =2-\sqrt{4-\varepsilon };$ con $0<\varepsilon < 4$ la definición se satisface.
...
\item $(\leftarrow)$ Ahora supongamos que $\lim_{{\bf x}\to {\bf a}} f_{i}(x_{1},x_{2},\cdots ,x_{n})=l_i$,
  ...
  \begin{gather*}
    \begin{aligned}
      ...
      &\leq \sum_{i=1}^{m}\left\vert f_{i}\left( x_{1},x_{2},\cdots ,x_{n}\right)-l_{i} \right\vert\\
      & < \sum_{i=1}^{m}\frac{\varepsilon }{m}=\varepsilon,
    \end{aligned}
  \end{gather*}
...
\textls[-20]{Este resultado permite hacer cálculos directos como se ilustra a continuación.}

\begin{parrafoitemize}
\item \textls[-20]{$\ds\lim_{t\rightarrow 0}
    \left\langle \frac{\sin t}{t},\sqrt{t},\frac{e^{t}-1}{t}\right\rangle=
    \left\langle \lim_{t\rightarrow 0}\frac{\sin t}{t}, \lim_{t\rightarrow 0}\sqrt{1+t},
  \lim_{t\rightarrow 0}\frac{e^{t}-1}{t}\right\rangle=\langle 1 ,1,1 \ra$}
\item Un límite con un campo vectorial.
  \begin{multline*}
    \textls[-20]{$\displaystyle \lim\limits_{(x,y) \rightarrow \left( 1,1\right) }\left\langle \frac{x-y}{\sqrt{x}-\sqrt{y}}, \frac{x^{3}-y^{3}}{x-y}\right\rangle =\lim\limits_{(x,y) \rightarrow (1,1) } \left\langle \sqrt{x}+\sqrt{y},x^{2}+xy+y^{2}\right\rangle$} \\
    \textls[-20]{$\displaystyle =\left\langle \lim\limits_{(x,y) \rightarrow \left( 1,1\right) }\left( \sqrt{x}+\sqrt{y}\right) ,\lim\limits_{(x,y) \rightarrow \left( 1,1\right) }\left( x^{2}+xy+y^{2}\right) \right\rangle = \left\langle 2,3\right\rangle$} \\
    \textls[-20]{$\displaystyle \lim_{(x,y) \rightarrow \left( 1,1\right) }\frac{x-y}{x^{2}-y^{2}}=\lim_{(x,y) \rightarrow \left( 1,1\right) }\frac{x-y}{\left( x-y\right) \left( x+y\right) }=\lim_{(x,y)\rightarrow\left(1,1\right) }\frac{1}{\left( x+y\right) }=\frac{1}{2}$}
  \end{multline*}
\end{parrafoitemize}

La imagen del conjunto $A$ por medio de la función $\ve{F}$, se denota por $\ve{F}(A)$, es decir
...
\begin{align*}
  ...
  (\mathbf{x}) \\
  &= f_{1}(\bm{x}) g_{1}\left( \mathbf{x}\right) +f_{2}(\mathbf{x}) g_{2}(\mathbf{x})
  +\cdots +f_{m}(\mathbf{x}) g_{m}(\mathbf{x}) \\
  &= \sum_{k=1}^{m}f_{k}(\mathbf{x}) g_{k}(\mathbf{x}) \\
  \left\Vert \ve{F}(\mathbf{x}) \right\Vert &= \left( f_{1}^{2}(\mathbf{x}) +f_{2}^{2}(\mathbf{x}) +\cdots
  +f_{m}^{2}(\mathbf{x}) \right) ^{1/2}=\sqrt{\sum_{k=1}^{m}f_{k}^{2}(\mathbf{x}) }\\
...
\begin{proof}
  ...
  \item Es consecuencia de la siguiente igualdad
    ...
    \end{multline*}
    Los términos de la derecha tienden a cero cuando $\mathbf{x}\to \mathbf{x}_{0}$.
...
\begin{defi} \index{Función!continua}
  Se dice que una función $\ve{F}:A\subseteq \mathbb{R}^n\to \mathbb{R}^m$ es continua en un punto
...
estará familiarizado con la definición de la derivada en un punto $a$,
\[f^{\prime}(a) =\lim_{h\rightarrow 0}\frac{f(a+h)-f(a) }{h}.\]
Al considerar la transformación lineal $A(h)=f^{\prime}(a)\cdot h$.
Entonces, la expresión anterior se reescribe como
\[\lim_{h\rightarrow 0}\frac{f\left( a+h\right) -f\left( a\right) -A\left( h\right) }{h}=0.\]

En el caso general una función ${\bf F}:\mathbb{R}^{n}\rightarrow \mathbb{R}^{m}$ se dice diferenciable en $a\in \mathbb{R}^{n}$ si existe una transformación lineal $\lambda :\mathbb{R}^{n}\rightarrow \mathbb{R}^{m}$, tal que
\[\lim_{h\rightarrow 0}\frac{\left\Vert {\bf F}(a+h)-{\bf F}(a)-\lambda(h)\right\Vert }{\left\Vert h\right\Vert }=0.\]
\textls[-5]{Es de anotar que $\left\Vert {\bf F}\left( a+h\right) -{\bf F}(a) -\lambda(h) \right\Vert$ representan la norma en $\mathbb{R}^{m}$ y $\left\Vert h\right\Vert $ es la norma en $\mathbb{R}^{n}$. \index{Función!diferenciable} Se dice que ${\bf F}$ es diferenciable en $A$, si es diferenciable para todo $a\in A$. La mencionada transformación lineal es la denominada \emph{matriz jacobiana} de ${\bf F}$ en $a$, que se denota por $D({\bf F})(a)$. Para ${\bf F}:\mathbb{R}^n\to {\mathbb R}^m$ y $a=(a_1,a_2,\cdots, a_n)$, es claro que ${\bf F}(a)=\langle f_1(a),f_2(a), \cdots, f_m(a) \rangle$. La entrada $ij$ de la matriz jacobiana está dada por:}
\begin{gather*}
...
\begin{teorema}
  ...
  \begin{prop}
    ...
    De esta forma, ${\bf F}$ es conservativo si y solo si se satisfacen las siguientes condiciones:
  ...
  \textls[-10]{De manera general, una aplicación que asigna a cada número real un vector en $\mathbb{R}^n$
  se denomina función vectorial, esto es ${\bf r}(t)= \langle r_1(t),r_2(t),\cdots, r_n(t)\ra$.}

  Por ejemplo, la función ${\bf r}(t)= \langle 3\cos(2t),1-\cos(t),2\sin(t)\ra$ está definida para
  $t\in [0,2\pi]$ y su gráfico en $\mathbb{R}^3$ se representa en la figura~\ref{curva_determinada}.

  \begin{figure}[H]
    \centering
    \caption{Curva determinada por ${\bf r}(t)= \langle 3\cos(2t),1-\cos(t),2\sin(t)\ra$}
    \label{curva_determinada}
    \includegraphics[height=4cm]{Fig/4.pdf} \hspace{5mm}
    \includegraphics[height=4cm]{Fig/5.pdf}
    \fuentepropia
  \end{figure}
  ...
  como caso particular de los teoremas \ref{preliminares3} y
  \ref{preliminares6}, con $n=1$.
  ...
  \begin{align*}
    ...
    \mathbf{u}(t) +\mathbf{u}(t) \cdot \mathbf{u}^{\prime }(t) \right)
    = \frac{\mathbf{u}^{\prime }(t)\cdot \mathbf{u}(t) }{\left\Vert \mathbf{u}(t)\right\Vert }.
  \end{align*}
  La derivada del producto cruz entre dos funciones vectoriales se evalúa como sigue.
  \begin{prop}
  ...
  \begin{proof}
    \textls[-15]{Si $\mathbf{u}(t)=\left\langle u_{1}(t),u_{2}(t),u_{3}(t) \right\rangle $ y
  ...
  \begin{gather*}
    \left( \left( \mathbf{u}(t)\times \mathbf{v}(t)\right)\cdot \mathbf{w}(t)\right)^{\prime} \\ = \left( \left( \mathbf{u}^{\prime}(t)\times \mathbf{v}(t)+\mathbf{u}(t)\times \mathbf{v}^{\prime}(t)\right) \cdot \mathbf{w}(t)\right) +\left( \left( \mathbf{u}(t)\times \mathbf{v}(t)\right)\cdot \mathbf{w}^{\prime}(t)\right).
  \end{gather*}
  ...
  \begin{tcolorbox}[title=\bf Curva parametrizada]
  ...
  \begin{tcolorbox}[title=\bf Superficie parametrizada]
...
\begin{tcolorbox}[title=\bf Parametrización de un sólido]
...
el cálculo de longitud de curvas, áreas, volumenes y centros de masa.
Se estudiará también el teorema de cambio de variables,
particularmente cuando se utilizan coordenadas polares, cilindricas o esféricas, pero se hará enfasis en
parametrizar la región de integración.
...
\[\sum_{i=1}^n f({\bf r}(t_i^*))\Delta \alpha_i=\sum_{i=1}^n f(x(t_i^*),y(t_i^*),z(t_i^*))\Delta \alpha_{i},\]
...
\int_a^b f({\bf r}(t))\Vert{\bf r}^\prime(t)\Vert dt=\lim_{n\to\infty}\left(\sum_{i=1}^n f({\bf r}(t_i^*))\Delta \alpha_i\right)
...
\begin{equation}
M=\int_a^b \delta({\bf r}(t))\Vert{\bf r}^\prime(t)\Vert dt
\end{equation}
...
\end{eqnarray*}
Las coordenadas del centro de masa de la varilla son:\index{Centro de masa}
\[(\overline{x},\overline{y},\overline{z})= \left(\frac{\mu_{yz}}{M},\frac{\mu_{xz}}{M},\frac{\mu_{xy}}{M}\right).\]
...
Además, \[\int_a^b{\bf f}(t)dt= \left\langle \int_a^b f_1(t)dt,\int_a^b f_2(t)dt,\cdots, \int_a^b f_n(t)dt\right\rangle.\]

\subsection{Integrales dobles} \label{integralesdobles}  \index{Integrales!dobles}
Presentaremos ahora la integral doble de una función de
dos variables $f(x,y)$ definida sobre una región del plano como el
límite de las aproximaciones a través de sumas de Riemann, posteriormente
se presentarán de manera similar las integrales triples.
La evaluación de las integrales dobles o triples se puede obtener
utilizando el teorema fundamental de cálculo; ver \cite{apostol_calculus_1999}.
...
Si $f(x,y)$ es una función continua definida sobre el rectángulo $R$,
subdividiremos a $R$ en $nm$ rectángulos más pequeños a partir
de rectas paralelas a los ejes coordenados. Para el intervalo $[a,b]$ consideremos
...
Denotaremos $\Delta x_{i}$ al ancho del $i-$ésimo subintervalo
sobre el eje $x$, y $\Delta y_{k}$ es el alto $k-$ésimo subintervalo sobre el eje $y$,
es decir $\Delta x_{i}=x_{i+1}-x_{i}$, de la misma forma $\Delta
y_{k}=y_{k+1}-y_{k}$. El área de uno de estos subintervalos de ancho $\Delta x_{i}$ y altura $\Delta y_{k}$ es
$\Delta A_{ik}=\Delta x_{i}\Delta y_{k}$.
La norma de la partición se denota por $\left\Vert P\right\Vert$ y es el máximo sobre todos los valores
$i$ y $k$, de todas las dimensiones de los rectángulos en que se divide la región $R$.

Cuando todos estos subintervalos son de igual longitud, los denotaremos por $\Delta x=\frac{b-a}{n}$ y $\Delta y=\frac{d-c}{m}$.
\index{Suma doble de Riemann}
Una suma de Riemann de $f(x,y) $ sobre el rectángulo $R$ se obtiene al considerar un punto cualquiera
$\left( x_{i}^{\ast },y_{k}^{\ast}\right) $, en $ik$-ésimo subrectángulo, esto
...
El valor de la función es este punto es $f\left( x_{i}^{\ast},y_{k}^{\ast }\right) $ y este valor se
multiplica por el área del $ik$-ésimo rectángulos; se suman todos estos productos para obtener una
expresión de la forma
\[S_{nm}=\sum_{i=1}^{n}\sum_{k=1}^{m}f\left( x_{i}^{\ast },y_{k}^{\ast}\right) \Delta x_{i}\Delta y_{k}.\]

Dependiendo de la selección $\left( x_{i}^{\ast },y_{k}^{\ast }\right) $,
es posible obtener diversos valores de $S_{nm}$.

Es de especial interés el conocer el comportamiento de las sumas de
Riemann cuando la norma de la partición tiende a cero, que equivale a
que el ancho y la altura de cada subrectángulo de $R$ tienden a cero.
Si el límite de las sumas $S_{nm}$ existe sin importar la selección
que se haga de $\left( x_{i}^{\ast },y_{k}^{\ast }\right) $, entonces
diremos que la función $f(x,y) $ es integrable sobre el rectángulo $R$
y este valor se denota por
$\ds \iint\limits_{R}f\left(x,y\right) dA$ o también $\ds \iint\limits_{R}f(x,y) dydx$.
...
&= \ds \iint\limits_{R}f(x,y) dA \label{integralesdobles1}
\end{align}
...
Si la función $f(x,y)$ es positiva sobre el rectángulo $R$ del plano $xy$ la integral doble de $f$ sobre $R$ puede
interpretarse como el volumen del sólido $E$ acotado en la base por $R$,
por la parte superior la superficie $z=f(x,y)$, y en los lados regiones contenidas en
los planos $x=a$, $x=b$, $y=c$, $y=d$. Nótese que el término
$f(x_{i}^{\ast },y_{k}^{\ast })\Delta x_{i}\Delta y_{k}$ corresponde al
volumen del paralelepípedo con área de la base $\Delta x_{i}\Delta y_{k}$ y altura $f(x_{i}^{\ast },y_{k}^{\ast });$
la suma $S_{nm}$ es una aproximación al volumen del sólido $E$.
Es por lo anterior que el volumen de $E$ se define como\index{Masa de una región}
\[\lim_{n,m \to \infty}\left(\sum_{i=1}^{n}\sum_{k=1}^{m}f\left( x_{i}^{\ast },y_{k}^{\ast}\right) \Delta x_{i}\Delta y_{k}\right)= \ds \iint\limits_{R}f(x,y) dA.\]
...
$\mu_{x}= \iint_R y f(x,y)dA$ y $\mu_{y}= \iint_R x f(x,y)dA$, los cuales determinan las coordenadas del
centro de masa de la región $R$, esto es:\index{Centro de masa}
\[(\overline{x},\overline{y})= \left(\frac{\mu_y}{M},\frac{\mu_x}{M}\right).\]

Si la región de integración en la fórmula \ref{integralesdobles1} es un rectángulo, esta se evalúa como
\[
...
\begin{figure}[H]
\centering
\caption{Regiones de integración en el plano}
\label{Regiones_integracion_plano}
\includegraphics[height=4cm]{Fig/7.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Fig/8.pdf}
\fuentepropia
\end{figure}
...
\begin{figure}[H]
\centering
\caption{Aproximaciones para el volumen bajo $f(x,y) = 8+2x-x^3-4y$}
\label{fig:aproximaciones_volumen}
\includegraphics[height=3.8cm]{Img/T_51a.png}\hspace{5mm}
\includegraphics[height=3.8cm]{Img/T_51b.png}\hspace{5mm}
\includegraphics[height=3.8cm]{Img/T_51c.png}
\fuentepropia
\end{figure}
...
por lo tanto, el volumen está dado por {\small $\ds \int_a ^b A(x) dx= \int_a ^b \int_c ^d f(x,y) dy dx $.} O de manera similar
{\small $\ds \int_c ^d A(y) dy= \int_c ^d \int_a ^b  f(x,y) dx dy$.}

Ejemplo de lo anterior es la siguiente integral
que representa el volumen del sólido en mención:
\[\ds \int_0^2\int_0^1 (8+2x-x^3-4y) dydx = \int_0^1 \int_0^2(8+2x-x^3-4y) dxdy.\]

\begin{figure}[H]
\centering
\caption[Secciones transversales bajo $z = 8+2x - x^3 - 4y$, con $x \in {[0,2]}$, $y \in {[0,1]}$]{Secciones transversales bajo $z = 8+2x - x^3 - 4y$,\\ con $x \in [0,2]$, $y \in [0,1]$}
\label{fig:secciones_transversales_bajo}
\includegraphics[height=2.8cm]{Img/T_52a.pdf}\hspace{3mm}
\includegraphics[height=2.8cm]{Img/T_52b.pdf}\hspace{3mm}
\includegraphics[height=2.8cm]{Img/T_52c.pdf}\hspace{3mm}
\includegraphics[height=2.8cm]{Img/T_52d.pdf}
\fuentepropia
\end{figure}
...
d,r\leq z\leq s\}.
\end{equation*}
Si $f(x,y,z)$ es una función continua definida sobre la caja rectangular $E$,
subdividiremos a $E$ en $lnm$ cajas rectangulares más pequeñas a
...
Denotaremos $\Delta x_{i}$ al ancho del $i$-ésimo subintervalo sobre el
eje $x$, $\Delta y_{j}$ es el largo $j$-ésimo subintervalo sobre el eje $y$, y $\Delta z_{k}$ es el alto $k$-ésimo subintervalo sobre el eje $z$, es decir $\Delta x_{i}=x_{i+1}-x_{i},\Delta y_{j}=x_{j+1}-x_{j}$, $\Delta
z_{k}=z_{k+1}-z_{k}$.
...
La norma de la partición se denota por $\left\Vert P\right\Vert $ y es
el máximo sobre todos los valores $i$, $j$ y $k$ de todas las
dimensiones de las cajas en que se divide la región $E$.

Una suma de Riemann de $f(x,y,z) $ sobre la caja rectangular $E$ se consigue al considerar un punto cualquiera
$\left( x_{i}^{\ast},y_{j}^{\ast },z_{k}^{\ast }\right) $, en la $ijk$-ésima caja
...
función es este punto es $f\left( x_{i}^{\ast },y_{j}^{\ast},z_{k}^{\ast }\right) $
y este valor se multiplica por el volumen de la $ijk$-ésima caja rectangular; se suman todos estos productos para obtener
una expresión de la forma
\begin{equation*}
S_{nml}=\sum_{i=1}^{n}\sum_{j=1}^{m}\sum_{k=1}^{l}f\left( x_{i}^{\ast},y_{j}^{\ast },z_{k}^{\ast }\right) \Delta x_{i}\Delta y_{j}\Delta z_{k}.
\end{equation*}
Dependiendo de la elección de $\left( x_{i}^{\ast },y_{j}^{\ast },z_{k}^{\ast }\right)$
es posible obtener diversos valores de $S_{nml}$.
De manera similar a como se definió la integral doble, si el límite de
las sumas $S_{nml}$ existe para toda elección que se haga de
$\left(x_{i}^{\ast },y_{j}^{\ast },z_{k}^{\ast }\right)$, entonces se dice que la
función $f(x,y,z) $ es integrable sobre la caja rectangular $E$, y este valor se denota por $\iiint\limits_{E}f(x,y,z) dV$, o también:
\[\iiint\limits_{E}f(x,y,z) dzdydx.\]
Lo anterior nos motiva a precisar este concepto.
...
%% TODO: ESPERAR DECISIÓN PARA REVIERTIR ASPECTO DE ESTAS ECUACIONES SI AHCE FALTA
...
\begin{align*} \int_{-1}^1\int_0^1\int_0^1  yz^2 e^{x} dzdydx &= \lim_{n,m,l \to \infty}\sum_{i=1}^n\sum_{j=1}^m\sum_{k=1}^l f(x_i,y_j,z_k)\Delta x \Delta y\Delta z \\
  &=\frac{1}{6} \left(e-\frac{1}{e}\right)
\end{align*}
\end{ejemplo}

\begin{ejemplo}
Para $f(x,y,z)= \cos (x) \sin (y)+z^2 \cos^2 (2 y)$ y $E$ el paralelepípedo determinado por
$[-\frac{\pi}{2},\frac{\pi}{2}]\times [0,\frac{\pi}{4}]\times[0,1]$; evaluar
\[\ds \iiint_Ef(x,y,z) dzdydx.\]
...
\Delta y&= \frac{\frac{\pi}{4}-0}{m}=\frac{\pi}{4m}\\
\text{y}\quad \Delta z&= \frac{1}{l}.
\end{align*}
Además,
...
\includegraphics[width=0.65\textwidth]{Fig/9.pdf}
\fuentepropia
\end{figure}
...
con $u \in [a,b]$ y $v \in [c,d]$, cuyas funciones componentes poseen derivadas parciales continuas en $[a,b] \times [c,d]$. Al realizar una partición de $\sigma$ en la forma
...
\includegraphics[height=3.5cm]{Fig/10.pdf}\hspace{5mm}
\includegraphics[height=3.5cm]{Fig/11.pdf}\\[5mm]
\includegraphics[height=3.5cm]{Fig/12.pdf}
\fuentepropia
\end{figure}
...
Una parametrización de una superficie es una función
$\phi:D\subseteq \mathbb{R}^{2}\rightarrow \mathbb{R}^{3}$.
La superficie $S$ correspondiente a la
función $\phi $ es su imagen $\phi \left( D\right) =S$ se escribe
...
La integración de una campo vectorial ${\bf f}$ sobre $S$ está dada por:
\begin{equation}
\iint_{S} {\bf f}(x,y,z) \cdot dS=\iint_{D} {\bf f} \left( \phi \left(
u,v\right) \right) \cdot \left( {\bf T}_{u}\times {\bf T}_{v}\right) dudv
\end{equation}
\textls[-15]{Una exposición más detallada de esta formulación puede encontrarse en \cite{marsden_calculo_2013}.}

Si en la expresión \ref{areasup} $f(x,y,z)=1$, esta integral se reduce al área $A$ de la superficie $D$, es decir:
\begin{equation}
A=\iint_{D} \left\| {\bf T}_{u}\times {\bf T}_{v}\right\| dudv \label{areasup2}
\end{equation}
\textls[-15]{Al término $\left\| {\bf T}_{u}\times {\bf T}_{v}\right\|dudv$ lo denominaremos elemento diferencial de área.}
...
Asumiendo que ${\bf r}$ es diferenciable, si $(u,v) $ son las coordenadas del rectángulo, los vértices
...
\end{eqnarray*}
El área de esta pequeña región está dada por:
\begin{equation}
...
\left\vert
\begin{array}{cc}\frac{\partial x}{\partial u}&\frac{\partial x}{\partial v}\\[.1cm]
\frac{\partial y}{\partial u} & \frac{\partial y}{\partial v}
\end{array} \right\vert
\end{equation}

En algunos libros como \cite{stewart_calculo_2018}, el término $\left\vert \frac{\partial\left(x,y\right)}{\partial(u,v)}\right\vert$
...
\begin{equation}
\iint\limits_{R}f(x,y) dA=\iint\limits_{R^{\ast }}f\left({\bf r}(u,v) \right) \left\vert \frac{\partial (x,y) }{\partial (u,v)}\right\vert dudv
\end{equation}
...
Consideremos un cubo en el sistema $uvw$, con aristas de longitud $\Delta u$ de largo, $\Delta v$ de ancho y $\Delta w$ de altura, que se transforma mediante la función ${\bf r}$.
Asumiendo que ${\bf r}$ es diferenciable,
si $(u,v,w) $ son las coordenadas de un vértice del cubo, entonces los vértices
adyacentes son $\left( u+\Delta u,v,w\right) $, $\left( u,v+\Delta v,w\right)$ y $\left( u,v,w+\Delta w\right) ;$ mediante la aplicación ${\bf r}$, el cubo se transforma en una región cuyo volumen se puede aproximar mediante un \mbox{paralelepípedo}.
Aplicando el teorema del valor medio, se sigue que
\begin{align*}
{\bf r}\left( u+\Delta u,v,w\right) -{\bf r}(u,v,w) &= \frac{{\bf r}\left( u+\Delta u,v,w\right) -{\bf r}(u,v,w) }{\Delta u}\Delta u \\
&\approx\frac{\partial {\bf r}}{\partial u}\left( u^{\ast},v,w\right) \Delta u
\end{align*}
\begin{align*}
{\bf r}\left( u,v+\Delta v,w\right) -{\bf r}(u,v,w) &= \frac{{\bf r}(u,v+\Delta v,w)-{\bf r}(u,v,w)}{\Delta v}\Delta v \\
&\approx\frac{\partial{\bf r}}{\partial v}\left( u,v^{\ast},w\right)\Delta v
\end{align*}
\begin{align*}
{\bf r}\left( u,v,w+\Delta w\right) -{\bf r}(u,v,w) &= \frac{{\bf r}\left(
u,v,w+\Delta w\right) -{\bf r}(u,v,w)}{\Delta w}\Delta w \\
&\approx\frac{\partial {\bf r}}{\partial w}\left( u,v,w^{\ast}\right) \Delta w.
\end{align*}
El volumen de esta región, $\Delta V$, está dado por:
\begin{equation}
...
En algunos libros el término $\left\vert \frac{\partial\left(x,y,z\right)}{\partial(u,v,w)}\right\vert$
se denomina el jacobiano de la transformación ${\bf r}$. \index{Jacobiano}
...
\includegraphics[height=4cm]{Fig/13.pdf} \vspace{5mm}
\includegraphics[height=4cm]{Fig/14.pdf}
\fuentepropia
\end{figure}
...
\label{integraldoblepolares}
\end{equation}
Utilizando el sistema de coordenadas cilíndricas $x=r\cos \theta $,
$y=r\sin \theta $, $z=z;$ o el sistema de coordenadas esféricas,
$x=\rho \cos \theta \sin \phi $, $y=\rho \sin \theta \sin \phi $, $z=\rho\cos \phi $,
con la figura~\ref{vectores_F_T_n} se ilustra la obtención del elemento diferencial de volumen en estos casos.

\begin{figure}[H]
\centering
\caption{Coordenadas esféricas}
\label{vectores_F_T_n}
\includegraphics[height=4cm]{Fig/15.pdf}
\fuentepropia
\end{figure}
...
0 & 0 & 1
\end{array}\right\vert = r\cos ^{2}\theta +r\sin ^{2}\theta = r,$$
...
\begin{figure}[H]
\centering
\caption{Elemento diferencial de volumen en coordenadas esféricas}
\label{fig:volumen-esfericas}
\includegraphics[width=0.4\textwidth]{Fig/16.pdf} \hspace{5mm}
\includegraphics[width=0.4\textwidth]{Fig/17.pdf}
\fuentepropia
\end{figure}

El jacobiano en el caso de las coordenadas esféricas corresponde a
$$\left\vert
\begin{array}{ccc}
...
\end{multline}
En el anexo~B el lector puede consultar las generalidades de otros sistemas de coordenadas.
\index{Integrales triples!en coordenadas Esféricas}

\begin{ejemplo}
Si $E$ es el sólido acotado por los paraboloides $z=2-x^2-y^2$ y $z=x^2+y^2$, evaluar la integral $\iiint_E y^2 dV$. La intersección de los paraboloides está dada por $x^2+y^2=1$, que corresponde a un círculo unitario en el plano $xy$; con estos elementos se plantea y evalúa esta integral:
...
se define el rotacional y la divergencia de ${\bf F}$ que se denotan por $\rot{\bf F}$ y $\dive{\bf F}$, y
están dados, respectivamente, por\index{Divergencia}\index{Rotacional}:

\[\rot{\bf F}(x,y,z)= \left\langle  \frac{\partial R}{\partial y}-\frac{\partial Q}{\partial z}, \frac{\partial P}{\partial z}-\frac{\partial R}{\partial x}, \frac{\partial Q}{\partial x}-\frac{\partial P}{\partial y} \right\rangle \]
y \[\dive{\bf F}(x,y,z)= \frac{\partial P}{\partial x}+\frac{\partial Q}{\partial y}+\frac{\partial R}{\partial z}.\]

\textls[-10]{Nótese que $\rot{\bf F}(x,y,z)$ es un campo vectorial y $\dive{\bf F}(x,y,z)$ es un campo escalar. Existe una manera de recordar brevemente dichos cálculos, en términos del operador $\nabla$, que se presentó en la sección \secref{diferencibilidad}.}
\begin{align*}
{\rm rot}{\bf F} &= \nabla \times{\bf F}  = \left\vert
\begin{array}{ccc}
\vi & \vj &  \vk \\  \frac{\partial }{\partial x}  & \frac{\partial }{\partial y}
& \frac{\partial }{\partial y}  \\ P & Q & R
\end{array}\right\vert\\
{\rm div}{\bf F} &= \nabla \cdot{\bf F}.
\end{align*}

\begin{ejemplo}
\textls[-15]{Para el campo vectorial ${\bf F}(x,y,z) =\left\langle xye^{z},z\cos (xy) ,\sin (xy) \right\rangle$,} es claro que
...
\end{ejemplo}
La integración de estos tipos especiales de funciones se considerarán en el capítulo \ref{chap:stokes}, \secref{chap:stokes},
