...
función $f(x,y,z) $ es integrable sobre la caja rectangular $E$, y este valor se denota por
$\ds\iiint\limits_{E}f(x,y,z) dV$, o también
\[\ds \iiint\limits_{E}f(x,y,z) dzdydx.\]
Lo anterior nos motiva a precisar este concepto.
...
\begin{multline*} \int_{-1}^1\int_0^1\int_0^1  yz^2 e^{x} dzdydx= \lim_{n,m,l \to \infty}\sum_{i=1}^n\sum_{j=1}^m\sum_{k=1}^l f(x_i,y_j,z_k)\Delta x \Delta y\Delta z \\ =\frac{1}{6} \left(e-\frac{1}{e}\right)
\end{multline*}
\end{ejemplo}
...
\textls[-10]{Nótese que $\rot{F}(x,y,z)$ es un campo vectorial y ${\rm div\,}{\bf F}(x,y,z)$ es un campo escalar. Existe una manera de recordar brevemente dichos cálculos, en términos del operador $\nabla$, que se presentó en la sección \secref{diferencibilidad}.}
\begin{eqnarray*}
...
\end{ejemplo}
La integración de estos tipos especiales de funciones se considerarán en el capítulo \secref{chap:stokes},
