...
\begin{figure}[H]
  ...
  \includegraphics[height=4cm]{Fig/170.pdf}
  \fuentepropia
\end{figure}
...
\begin{gather*}
  ...
  \right\rangle \cdot
  \left\langle f'(t_i^{\ast}),\; g'(t_i^{\ast\ast}) \right\rangle \,
  \Delta t_i.
\end{gather*}
...
\begin{ejemplo} \label{integraldelinea}
  ...
  \begin{itemize}
    \item ${\bf r}(t) =\left\langle t^{2},t,2-t\right\rangle $, con $t\in[-1,1]$. En este caso,\\ ${\bf F}\left({\bf r}(t) \right) \cdot {\bf r}^{\prime }(t)= \left\langle t^{3},t\left( 2-t\right) ,-t^{4}\right\rangle \cdot \left\langle 2t,1,-1\right\rangle = 3t^{4}-t^{2}+2t$.
      \begin{gather*}
        \int_{C}{\bf F}\cdot d{\bf r} =\int_{-1}^{1}{\bf F}\left({\bf r}(t) \right) \cdot {\bf r}^{\prime }(t) dt
...
\begin{ejemplo}
\textls[-25]{Evaluar la integral de línea de ${\bf F}\left( x,y,z\right) =\left\langle y^{2}-2xz,2xy,-x^{2}\right\rangle$} sobre la curva que une los puntos $\left( -1,1,3\right) $ con $\left( 1,1,1\right)$, considerando las siguientes parametrizaciones.
...
\begin{ejemplo}
Evaluar la integral de línea de
\[{\bf F}\left( x,y,z\right) =\left\langle 4 x^3 y,x^4+3 y^2 z,y^3 \right\rangle\]
sobre la curva $\alpha$ parametrizada por
\[{\bf r}(t)=\langle 2 t-1,t+2,9 t^2 \rangle \text{ con } t \in [-1,2].\]
...
\[Phi][x_,y_,z_]=u[x,y,z]/.First@DSolve[{D[u[x,y,z],x]
==P,D[u[x,y,z],y]==Q,D[u[x,y,z],z]==R},u[x,y,z],x,y,z]
...
\includegraphics[height=3cm]{Fig/171.png}
\fuentepropia
\end{figure}
...
En\cite{acosta_gempeler_doce_2020} se define la densidad rotacional como la circulación por unidad de área, y la densidad de expansión es el flujo por unidad de área. \mbox{Analizaremos} estos conceptos en campos vectoriales bidimensionales. Posteriormente, se extienden a campos tridimensionales.
...
Las razones de flujo son positivas o negativas, dependiendo de las componentes de ${\bf F} $. Aproximamos la razón de circulación neta alrededor de la frontera rectangular de $R$ sumando las razones de flujo a lo largo de los cuatro lados, como lo indican los siguientes productos punto.
\begin{align*}
\text{Arriba} \quad &  {\bf F} \left( x,y+\Delta y\right) \cdot (-\vi) \Delta x=-P\left( x,y+\Delta y\right) \Delta x \\
\text{Abajo} \quad & {\bf F} (x,y) \cdot \vi\Delta x=P(x,y) \Delta x \\
\text{Lado derecho} \quad & {\bf F} ( x+\Delta x,y) \cdot \vj\Delta y=Q\left(x+\Delta x,y\right) \Delta y \\
\text{Lado izquierdo} \quad & {\bf F} (x,y) \cdot \left( -\vj\right) \Delta y=-Q(x,y) \Delta y.
\end{align*}

\textls[-15]{Al sumar estos elementos y aplicar el teorema del valor medio, se sigue que}
\begin{gather}
...
\begin{defi}
\textls[-15]{Sea ${\bf F} (x,y)=P(x,y) \vi+Q(x,y) \vj$ un campo vectorial cuyas funciones componentes poseen primeras derivadas parciales continuas.
La densidad rotacional del campo vectorial ${\bf F} (x,y)$ en el punto $(x,y)$ está dada por:}
...
De manera aproximada, la razón del flujo neto a través de la frontera del rectángulo $R$ corresponde a la suma de las razones de flujo a través de los cuatro lados de $R$. En el lado inferior, el flujo es aproximadamente el producto de la densidad de flujo de ${\bf F}$ en el punto medio por la longitud del segmento, es decir ${\bf F} (x,y) \cdot \left( -\vj\right) \Delta x$. Por lo anterior, se consideran los siguientes productos punto para el flujo de salida.
\begin{align*}
\text{Arriba}\quad & {\bf F} \left( x,y+\Delta y\right) \cdot \mathbf{j}\Delta x=Q\left(x,y+\Delta y\right) \Delta x \\
\text{Abajo}\quad & {\bf F} (x,y) \cdot \left( -\vj\right) \Delta x=-Q\left(x,y\right) \Delta x \\
\text{Lado derecho}\quad & {\bf F} ( x+\Delta x,y) \cdot \vi\Delta y=P\left(x+\Delta x,y\right) \Delta y \\
\text{Lado izquierdo}\quad & {\bf F} (x,y) \cdot \left( -\vi\right) \Delta y=-P(x,y) \Delta y.
\end{align*}

\textls[-10]{Al sumar estos elementos y aplicar el teorema del valor medio, se sigue que}
\begin{gather}
...
\textls[-5]{Sea $C$ una curva suave por tramos, cerrada y simple que encierra una región $R$ en el plano. Sea ${\bf F} (x,y)=P(x,y)\vi+Q(x,y)\vj$ un campo vectorial donde $P(x,y)$ y $Q(x,y)$ tienen primeras derivadas parciales continuas en una región abierta que contiene a $R$. Entonces, la circulación en contra de las manecillas del reloj de ${\bf F}$ alrededor de $C$ es la doble integral de $({\rm rot}{\bf F})\cdot \vk$ sobre la región $R$.}\index{Región abierta} \index{Teorema!de Green}
\begin{equation}
...
Integrate[A*r,{\[Theta],0,2*Pi},{r,0,a*(1+Cos[\[Theta]])}]
\end{Verbatim}
\end{tcolorbox}
...
Si las funciones $x(u,v) $, $y(u,v) $ y $z(u,v)$ son diferenciables, se dice que ${\bf r}$ es diferenciable. Si ${\bf r}$ es diferenciable en $\left( u_{0},v_{0}\right) $, al fijar $u$, se obtiene que la función ${\bf r}\left( u_{0},v\right) $ posee como imagen una curva en $\mathbb{R}^{3}$. Los vectores ${\bf r}_{u}$ y ${\bf r}_{v}$ son tangentes a esta curva en $\left(u_{0},v_{0}\right)$; de modo que ${\bf r}_{u}\times {\bf r}_{v}$ es normal a la superficie en $\left( u_{0},v_{0}\right)$. Se dice que la superficie es \emph{suave} \index{Superficie!suave} si ${\bf r}_{u}\times {\bf r}_{v}\neq {\bf 0}$, en todo punto de $S$ (esto puede depender de la parametrización elegida).
...
\includegraphics[height=4cm]{Fig/174.pdf}
\fuentepropia
\end{figure}
...
\includegraphics[height=4cm]{Fig/175.pdf}
\fuentepropia
\end{figure}
...
\includegraphics[height=4cm]{Fig/176.pdf}
\fuentepropia
\end{figure}
...
\includegraphics[height=4cm]{Fig/177.pdf}
\fuentepropia
\end{figure}
...
\includegraphics[height=4cm]{Fig/178.pdf}
\fuentepropia
\end{figure}
...
\centering
\caption{Campo vectorial ${\bf r}_u \times {\bf r}_v$}
\includegraphics[height=4cm]{Fig/179.pdf}
\fuentepropia
\end{figure}
...
\includegraphics[height=4cm]{Fig/180.pdf}
\fuentepropia
\end{figure}
...
\includegraphics[height=4cm]{Fig/181.pdf}
\fuentepropia
\end{figure}
...
\includegraphics[height=4cm]{Fig/182.pdf}
\fuentepropia
\end{figure}
...
\textls[-20]{Sea ${\bf F}(x,y,z)= P(x,y,z)\vi+ Q(x,y,z)\vj + R(x,y,z)\vk$}
...
\end{teorema}
La prueba se puede conseguir en\cite{marsden_calculo_2013} o\cite{apostol_calculus_1999}.
...
Para el campo vectorial ${\bf F}(x,y,z)=\la y^3,z^3,x^3 \ra$, evaluar $\iint_S {\rm rot\,}{\bf F}\cdot d{\bf S}$ sobre la superficie del paraboloide $x=3-\left( y-2\right) ^{2}-\left(z-1\right) ^{2}$ acotada por el cilindro $\left( y-2\right) ^{2}+\left(z-2\right) ^{2}=4$. Esta región se consideró en la página~\pageref{curvas2}.

\begin{figure}[H]
\centering
\caption{Superficie sobre el paraboloide interior al cilindro}
\includegraphics[height=3.6cm]{Img/70b.pdf} \hspace{2mm}
\includegraphics[height=3.6cm]{Img/T_22a.pdf} \hspace{2mm}
\includegraphics[height=3.6cm]{Img/T_22b.pdf}
\fuentepropia
\end{figure}
...
b=SliceVectorPlot3D[{1,-4+2y,-2+2z},x==-3+(y-2)^2+(z-1)^2,
{x,-3,7},{y,0,4},{z,0,4.2},VectorScale->{0.08,Scaled[0.4]},
RegionFunction->Function[{x,y,z},(y-2)^2+(z-2)^2<4],
VectorStyle->Black,VectorPoints->18,PlotStyle->Opacity[0]];
c=ParametricPlot3D[{2+4*Sin[t],2+2*Cos[t],2+2*Sin[t]},
{t,0,2*Pi},PlotStyle->{Blend[{Red,Black},1/2],Thickness->0.01},
PlotPoints->80];
Show[b,a,c]
\end{Verbatim}
\end{tcolorbox}

\textls[-20]{Con el campo vectorial ${\bf F}$ se verifica que ${\rm rot\,}{\bf F}(x,y,z)=\la -3z^2,-3 x^2,-3 y^2\ra$. La proyección de la superficie en el plano $yz$ se parametriza mediante las expresiones $y = 2+2u \cos (v)$, $z = 2+2u \sin (v)$. Reemplazando en la expresión que define el paraboloide se sigue que $x = -3+ (y-2)^2+(z-1)^2=-2+4u^2+4u\sin(v)$. En resumen, para $u\in [0,1],\, v\in [0,2\pi]$, la siguiente función parametriza la superficie sobre el paraboloide con la restricción mencionada.}
...
{\rm rot\,}{\bf F}({\bf r}(u,v)) \cdot ({\bf r}_u \times {\bf r}_v) = 48u\bigg(2(1+u\cos(v))^2(1+2u\sin(v))\\
-(1+u\sin(v))^2+u\cos(v)(4u^2-2+4u\sin(v))^2 \bigg ).
\end{gather*}
Con lo cual,
...
r={x,y,z};
Integrate[Dot[G,Cross[D[r,u],D[r,v]]],{v,0,2*Pi},{u,0,1}]
\end{Verbatim}
\end{tcolorbox}
...
y también
\begin{gather*}
{\bf F}\big({\bf r}(t)\big) \cdot {\bf r}'(t) =
16\cos(t)\bigg(2\big(1 + \cos(t)\big)^3 + \big(1 + 2\sin(t)\big)^3\bigg)
+ 16\sin(t)\big(1 + \sin(t)\big)^3.
\end{gather*}
...
\includegraphics[height=3.4cm]{Img/72a.pdf} \hspace{2mm}
\includegraphics[height=3.4cm]{Img/T_26a.pdf} \hspace{2mm}
\includegraphics[height=3.4cm]{Img/T_26b.pdf}
\fuentepropia
\end{figure}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
a=SliceVectorPlot3D[{2*x,2*y,-2},{2*z==x^2+y^2},{x,-1,3},{y,-3,1},
{z,0,6},RegionFunction->Function[{x,y,z},(x-1)^2+(y+1)^2<=4],
VectorStyle->Black,VectorScale->{0.1,Scaled[0.2],Automatic},
VectorPoints->15];
...
\end{align*}
De modo que
\[{\rm rot\,}{\bf F} \cdot ({\bf r}_u \times {\bf r}_t) = 8u(-1-u\cos(t)+u\sin(t)+u^2\sin(2t)),\]
entonces
\begin{gather*}
...
Integrate[Dot[G,Cross[D[r,u],D[r,t]]],{t,0,2*Pi},{u,0,1}]
\end{Verbatim}
\end{tcolorbox}
...
\includegraphics[height=3.5cm]{Fig/183.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Fig/184.pdf}
\fuentepropia
\end{figure}
...
$z=\rho \sin \theta \sin \phi $ para construir una parametrización de $S$. Tendremos en cuenta que la ecuación $x^{2}+y^{2}+z^{2}=2x$ se reescribe como $\rho =2\cos \phi $,
...
y
\begin{gather*}
{\bf F}({\bf u}(\theta,\phi))\cdot ({\bf u}_{\theta }\times {\bf u}_{\phi })
= -\sin 4\phi -2\left( \cos \theta +\sin \theta \right) \sin^{2}\left( 2\phi \right).
\end{gather*}
...
\includegraphics[height=3.5cm]{Img/T_18a.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/T_18b.pdf}
\fuentepropia
\end{figure}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
a=SliceVectorPlot3D[{1,1,1},{x^2+y^2+z^2==2*x},{x,0,3/2},{y,-1,1},
{z,-1,1},VectorPoints->12,VectorStyle->Black,
VectorScale->{0.12,Scaled[0.2],Automatic},
Ticks->{{0,1},{-1,0,1},{-1,0,1}},PlotStyle->Opacity[0]];
b=ParametricPlot3D[{1+Sqrt[3]/2,Cos[\[Theta]]/2,
Sin[\[Theta]]*Sin[2*Pi/6]},{\[Theta],0,2Pi},PlotStyle->Blue];
...
\begin{eqnarray*}
{\bf F}(\vec{\alpha}(t))\cdot d\vec{\alpha}
...
\includegraphics[height=4cm]{Fig/185.pdf}
\fuentepropia
\end{figure}

\textls[-15]{Utilizaremos la parametrización ${\bf r}\left( u,v\right) =\left\langle 4\cos u\sin v,2\sin u\sin v,4\cos v\right\rangle $, con $u\in \lbrack 0,2\pi ]$, $v\in \lbrack 0,\pi/2]$.}
...
\textls[-10]{Con los elementos anteriores es sencillo ver que ${\rm rot}\,{\bf F}= \left\langle y\sin yz,0,0\right\rangle$, además
${\rm rot}\,{\bf F}({\bf r}(u,v)) \cdot ({\bf r}_{u}\times {\bf r}_{v})= -8\sin(2u)\sin^3(v)\sin(4\sin(u)\sin(2v))$,
por lo tanto,}
\begin{gather*}
...
$\vec{\alpha}(t) =\left\langle 4\cos t,2\sin t,0\right\rangle$, $t\in \lbrack 0,2\pi ]$.
El mismo cálculo se puede evaluar usando una integral de línea, en este caso
${\bf F}(\vec{\alpha}(t))\cdot \vec{\alpha}^{\prime}(t)=\la e^{4\cos t},\cos(0),0\ra\!\cdot\la -4\sin t,2\cos t,0\ra$,
por lo tanto,
\[\int_{C}{\bf F}\cdot d\vec{\alpha} = \int_{0}^{2\pi}{\bf F}(\vec{\alpha}(t))\cdot
\vec{\alpha}^{\prime }\left( t\right) dt = \int_{0}^{2\pi}\left( 2\cos t-4e^{4\cos t}\sin t\right) dt= 0.\]
...
\includegraphics[height=3.2cm]{Img/T_4a.pdf} \hspace{5mm}
\includegraphics[height=3.2cm]{Img/T_4b.pdf}
\fuentepropia
\end{figure}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
a=ContourPlot3D[3-x^2-y^4+2*y^2==z,{x,-2,2},{y,-2,2},{z,0,4.2},
Mesh->False,PlotPoints->60];
Show[a,SliceVectorPlot3D[{2*x,2*y,z},{3-x^2-y^4+2*y^2==z},
{x,-2,2},{y,-2,2},{z,0,4.4},VectorStyle->Black,
VectorScale->{0.12,Scaled[0.3],Automatic},VectorPoints->12,
PlotStyle->Opacity[0]]]
\end{Verbatim}
\end{tcolorbox}

La ecuación $z=3-x^{2}-y^{4}+2y^{2}$ determina la superficie $S$
que interseca al plano $xy$, en una curva caracterizada por la expresión
$4=x^{2}+\left( y^{2}-1\right) ^{2}$. Esta región se muestra enseguida.

\begin{figure}[H]
\centering
\caption{$z=3-x^2-y^4+2y^2$ y su proyección en el plano $xy$}
\includegraphics[height=3.2cm]{Fig/186.pdf} \hspace{5mm}
\includegraphics[height=3.2cm]{Fig/187.pdf}
\fuentepropia
\end{figure}

Es claro que si $x=0$, entonces $y=\pm \sqrt{3}$, esta región se puede parametrizar formando segmentos rectilíneos entre dos puntos de la forma $A$ y $B$, que son de forma $A=\left( -\sqrt{3-t^{4}+2t^{2}},t\right)$, $B=\left( \sqrt{3-t^{4}+2t^{2}},t\right)$. Una combinación convexa entre $A$ y $B$, permite parametrizar la proyección de la superficie sobre el plano $xy$, esto es $\left( 1-u\right)A+uB=\left(\left(2u-1\right)\sqrt{2t^{2}-t^{4}+3},t\right)$. La coordenada $z$ sobre la superficie, se obtiene al reemplazar $x=(2u-1)\sqrt{2t^{2}-t^{4}+3}$ y $y=t$, en $z=3-x^{2}-y^{4}+2y^{2}$ obteniendo $z=4u(1-u)(3-t^{4}+2t^{2})$. Por lo anterior, la superficie se describe por medio de la función
...
\begin{gather*}
{\bf r}_{u} \times {\bf r}_{t} = \left\langle 4(1 - 2u)(t^{2} + 1)(t^{2} - 3),\;
\frac{-8t(t^6 - 3t^4 - t^2 + 3)}{\sqrt{3 - t^{4} + 2t^{2}}},\;
2\sqrt{3 - t^{4} + 2t^{2}} \right\rangle.
\end{gather*}

\textls[-15]{El término ${\bf F}\left({\bf r}(u,v)\right) \cdot \left({\bf r}_{u} \times {\bf r}_{v}\right)$ se puede simplificar de la siguiente manera:}
\[
8\sqrt{2t^{2} - t^{4} + 3} \left(3 + t^2(t^2u + t^2 - 6u) + 3u^2(2t^{2} - t^{4} + 3) - 9u\right),
\]
de modo que el flujo de ${\bf F}$ sobre $S$ está dado por:
\[
\int_{-\sqrt{3}}^{\sqrt{3}} \int_{0}^{1}{\bf F}\left({\bf r}(u,v)\right) \cdot
\left({\bf r}_{u} \times {\bf r}_{v}\right)\, du\, dt \approx 136.205.
\]
...
Evalúe $\iint_S \rot{\bf F} \cdot d{\bf S}$, en donde ${\bf F}(x,y,z)=\left\langle y^{2},yz-x,xz-y\right\rangle $ y $S$ es la superficie acotada por las figuras de las expresiones $x=z^2$ y $x=4y-y^2$. Esta superficie se presenta en la página \pageref{curvas3}.

En la figura~\ref{xzcon} se muestra el sólido y la representación de los campos vectoriales $\la -1,0,-2z\ra$ y  $\la 1,-4+2y,0\ra$, que determinan la orientación hacia afuera de la superficie.

\begin{figure}[H]
\centering
\caption{$x = z^2$ con $(y-2)^2+z^2 \leq 4$}\label{xzcon}
\includegraphics[height=3.5cm]{Img/53e.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/53h.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/53g.pdf}
\fuentepropia
\end{figure}
...
b=SliceVectorPlot3D[{-1,0,2*z},{x==z^2},{x,0,4.5},{y,0,4},
{z,-2.5,2.5},RegionFunction->Function[{x,y,z},(y-2)^2+z^2<=4],
VectorScale->{0.14,Scaled[0.2],Automatic},
VectorPoints->18,VectorStyle->Black];
Show[b,a]
c=ParametricPlot3D[{4*Sin[t]^2,2+2*Cos[t],2*Sin[t]},{t,0,2*Pi},
PlotStyle->{Black,Thickness[0.01]}];
d=SliceVectorPlot3D[{1,-4+2*y,0},{x==4*y-y^2},{x,-0.5,4},
{y,-0.5,5},{z,-2,2},RegionFunction->Function[{x,y,z},
(y-2)^2+z^2<=4],VectorScale->{0.2,Scaled[0.2],Automatic},
...
Del campo vectorial se  sigue que $\rot{\bf F}= \left\langle -y-1,-z,-2y-1\right\rangle $.
...
$\rot{\bf F}\left({\bf r}\left(u,v\right)\right) = \left\langle -2u\cos v-3,-2u\sin v,-4u\cos v-5\right\rangle $.
...
\end{align*}
Además,
\[\rot{\bf F}\left({\bf r}(u,v)\right)\cdot\left({\bf r}_{u}\times{\bf r}_{v}\right)=-4 u(4u^2\cos(2v)-4u^2+4 u \cos (v)+5), \]
entonces,
\begin{equation}\label{stokes6}
\int_{0}^{2\pi }\int_{0}^{1}\left( -4 u(4u^2\cos(2v)-4u^2+4 u \cos (v)+5)\right) dudv= -12\pi
\end{equation}
\textls[-20]{Las instrucciones requeridas en \verb"Mathematica" se presentan a continuación:}

\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
G=Curl[{y^2,y*z-x,x*z-y},{x,y,z}]
r={y,z,x}={2+2*u*Cos[v],2*u*Sin[v],z^2}
Integrate[Dot[G,Cross[D[r,u],D[r,v]]],{u,0,1},{v,0,2Pi}]
\end{Verbatim}
\end{tcolorbox}
...
Por lo tanto, $\rot{\bf F}\left({\bf r}\left( u,v\right)\right) =\big \langle -2u\cos v-3,-2u\sin v,-4u\cos v-5\big \rangle$.
...
\end{eqnarray*}
además, \[\rot{\bf F}\left({\bf r}(u,v) \right)\cdot\left({\bf r}_{u}\times {\bf r}_{v}\right)= -4u\left( 4u^{2}\sin 2v+2u\cos v+3\right),\]
entonces,
...
\includegraphics[height=3.5cm]{Img/T_33a.pdf} \hspace{2mm}
\includegraphics[height=3.5cm]{Img/T_33c.pdf} \hspace{2mm}
\includegraphics[height=3.5cm]{Img/T_33b.pdf}
\fuentepropia
\end{figure}
...
a=ContourPlot3D[z==(x+y^2)*Exp[1-x^2-y^2],{x,-2,2},{y,-2,2},
{z,-4/3,4/3},MeshStyle->Gray,BoxRatios->{1,1,0.75}];
b=ListSliceVectorPlot3D[datos,ImplicitRegion[z==(x+y^2)
*Exp[1-x^2-y^2],{x,y,z}],DataRange->{{-2,2},{-2,2},{-4/3,3/2}},
VectorPoints->15,VectorStyle->{Thickness->0.005,Black,
Thickness->0.004},PlotPoints->60,PlotStyle->{Opacity[0]}];
...
Puede verse que
\begin{gather*}
{\bf r}_u \times {\bf r}_t = \bigg \langle 4 e^{1-4 u^2} u \left(4 u^3 \cos (t)-4 u^3 \cos (3 t)+4 u^2 \cos (2 t)+4 u^2-1\right),\\
16e^{1-4 u^2} u^2 \sin (t) \left(2 u^2 \cos (2t)-2 u \cos (t)-2 u^2+1\right),4 u\bigg \rangle;
\end{gather*}
además,
$\rot{\bf F}\cdot ({\bf r}_u \times {\bf r}_t)= 8u$, entonces:
\begin{equation} \label{stokes9}
\iint_S \rot{\bf F}\cdot {\bf n} d \sigma= \int_0^1\int_0^{2\pi} 8u dtdu =8\pi
\end{equation}
Este cálculo se hace en \verb"Mathematica" con las siguientes instrucciones:

\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
G=Curl[{x-y,x+y,2*z},{x,y,z}]
r={x,y,z}={2*u*Cos[t],2*u*Sin[t],(x+y^2)*Exp[1-x^2-y^2]};
Integrate[Dot[G,Cross[D[r,u],D[r,t]]],{u,0,1},{t,0,2Pi}]
\end{Verbatim}
\end{tcolorbox}
...
\includegraphics[height=3.5cm]{Img/T_5a.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/T_5b.pdf}
\fuentepropia
\end{figure}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
SliceVectorPlot3D[{y*z,-x*z,-x*y},{z==Log[x^2+y^2]},{x,-3,3},
{y,-3,3},{z,-1,2},VectorPoints->14,
RegionFunction->Function[{x,y,z},0.1<x^2+y^2<9],
VectorStyle->Black,VectorScale->{0.16,Scaled[0.22],Automatic}]
\end{Verbatim}
\end{tcolorbox}
...
\begin{gather*}
{\bf F}({\bf r}(u,v)) = \left\langle u\sin v\ln u^{2},-u\cos v\ln u^{2},-u^{2}\cos v\sin v \right\rangle
\end{gather*}
...
\subsection{Flujo sobre la superficie de tipo exponencial}
...
\includegraphics[height=3cm]{Img/T_6b.pdf} \hspace{5mm}
\includegraphics[height=3cm]{Img/T_6a.pdf}
\fuentepropia
\end{figure}
...
\textls[-10]{La superficie que se considera se puede parametrizar de la siguiente manera ${\bf u}\left(x,y\right) =\la x,y,xe^{1-x^{2}-y^{2}}\ra $, con $x\in[-2,2]$, $y\in[-2,2]$. De esta función se obtienen los siguientes vectores:}
...
${\bf F}\left({\bf u}\left( x,y\right) \right) =\la x^{2},-y,1+xe^{1-x^{2}-y^{2}}\ra$ y
...
\includegraphics[height=3cm]{Img/T_1a.pdf} \hspace{5mm}
\includegraphics[height=3cm]{Img/T_1b.pdf}
\fuentepropia
\end{figure}
...
La ecuación del elipsoide se reescribe en la forma $\frac{x^{2}}{4}+y^{2}+z^{2}=1$, y se puede parametrizar multiplicando por $2$ la componente $x$ de la expresión correspondiente a las ecuaciones en coordenadas esféricas, que describen la esfera unitaria. Es por lo anterior que utilizaremos la función
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
Integrate[z+1,{x,-2,2},{y,-Sqrt[1-x^2/4],Sqrt[1-x^2/4]},
{z,-Sqrt[1-x^2/4-y^2],Sqrt[1-x^2/4-y^2]}]
\end{Verbatim}
\end{tcolorbox}
...
\includegraphics[height=3.5cm]{Img/T_2a.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/T_2b.pdf}
\fuentepropia
\end{figure}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
SliceVectorPlot3D[{x*z,x*(y+1),z*(y+1)},{4-x^2-y^2==z},{x,-2,2},
{y,-2,2},{z,0,4},VectorPoints->12,VectorStyle->Black,
VectorScale->{0.16,Scaled[0.22],Automatic},
Ticks->{{-2,0,2},{-2,0,2},{0,2,4}}]
\end{Verbatim}
\end{tcolorbox}
...
\[{\bf r}\left(u,v\right) = \left\langle u\cos v,u\sin v,0\right\rangle ,\, u\in [0,2],\,v\in \lbrack 0,2\pi ].\]
Es sencillo ver que
...
\begin{gather*}
  {\bf r}_{u} = \left\langle \cos v, \sin v, 0 \right\rangle
\end{gather*}
y
\begin{gather*}
...
Es claro que
\[
{\bf F}\left({\bf r}(u,v)\right) \cdot \left({\bf r}_{u} \times {\bf r}_{v}\right) = 0;
\]
...
${\bf r}\left(u,v\right) =\left\langle u\cos v,u\sin v,4-u^{2}\right\rangle,\, u\in [0,2],\, v\in \lbrack 0,2\pi ]$.
...
La figura~\ref{fig:stokessuperficies} muestra el sólido.
%\clearpage
%
...
\label{fig:stokessuperficies}
\includegraphics[height=3cm]{Img/T_T1.pdf} \hspace{5mm}
\includegraphics[height=3cm]{Img/T_T2.pdf}
\fuentepropia
\end{figure}
...
\includegraphics[height=3cm]{Img/T_21a.pdf} \hspace{2mm}
\includegraphics[height=3cm]{Img/T_21b.pdf} \hspace{2mm}
\includegraphics[height=3cm]{Img/T_21c.pdf}
\fuentepropia
\end{figure}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
a=RegionPlot3D[(4*z-9)/9<=y<=(3-9*x^2-4*z^2+4*z)/9&&9*x^2+4*z^2<=12,
{x,-2,2},{y,-2,1},{z,-2,2},PlotPoints->90,Mesh->False];
b=SliceVectorPlot3D[{x*z,x*(y+1),z(y+1)},
{4*z-9*y==9,9*y==3-9*x^2-4*z^2+4*z},{x,-2,2},{y,-2,1},{z,-2,2},
RegionFunction->Function[{x,y,z},9*x^2+4*z^2<12],
VectorPoints->20,VectorStyle->Black,PlotPoints->60,
VectorScale->{0.3,Scaled[0.4],Automatic},PlotStyle->Opacity[0]]
Show[a,b]
\end{Verbatim}
\end{tcolorbox}
...
{\bf r}_{u}\times {\bf r}_{v}&= \left\langle 0,-2u,\frac{8}{9}u\right\rangle.
\end{align*}
...
{\bf F}\left( {\bf r}\left( u,v\right) \right) \cdot \left( {\bf r}_{u}\times {\bf r}_{v}\right) &=\frac{32}{27}u^{3}\sin ^{2}v-\frac{16}{9}u^{3}\sin v\cos v.
\end{align*}

De tal modo que el flujo sobre la porción del plano es
\begin{gather} \label{T1}
\iint_{S_1} {\bf F} \cdot d\boldsymbol{\sigma} = \int_{0}^{2\pi }\int_{0}^{1} \frac{16u^3 \sin v}{9} \left(\frac{2}{3}\sin v-\cos v \right) dudv= \frac{8}{27}\pi
\end{gather}
...
{z,-Sqrt[12-9*x^2]/2,Sqrt[12-9*x^2]/2},
{y,4z/9-1,(3+4z-9*x^2-4z^2)/9}]
\end{Verbatim}
\end{tcolorbox}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
SliceVectorPlot3D[{y^2,-x*y,z},{4==x^2+y^2+z^2},{x,-2,2},{y,-2,2},
{z,-2,2},VectorPoints->10,VectorPoints->12,VectorStyle->Black,
...
\includegraphics[height=3.5cm]{Img/T_3a.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/T_3b.pdf}
\fuentepropia
\end{figure}
...
El proceso anterior se puede hacer en \verb"Mathematica" utilizando las siguientes instrucciones:

\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
F={y^2,-x*y,z};
r={x,y,z}={2*Cos[u]*Sin[v],2*Sin[u]*Sin[v],2*Cos[v]};
Integrate[Dot[Cross[D[r,v],D[r,u]],F],{u,0,2*Pi},{v,0,Pi}]
\end{Verbatim}
\end{tcolorbox}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
Integrate[(1-\[Rho]*Sin[\[Phi]]*Cos[\[Theta]])*\[Rho]^2*Sin[\[Phi]],
{\[Rho],0,2},{\[Phi],0,Pi},{\[Theta],0,2*Pi}]
\end{Verbatim}
\end{tcolorbox}
...
\item Hallar el flujo neto del campo vectorial ${\bf F}(x,y,z) = \la y^2,x^2,z \ra $ sobre la superficie cerrada del sólido acotado por los paraboloides con ecuaciones:
\begin{gather*}
...
\includegraphics[height=3.5cm]{Img/T_8b.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/T_8d.pdf}
\fuentepropia
\end{figure}
...
b=SliceVectorPlot3D[{y^2,x^2,z},{2*z==6-2*x^2-y^2,z==x^2+y^2-3},
{x,-2,2},{y,-2,2},{z,-3,3},VectorPoints->20,
VectorScale->{0.15,Scaled[0.25],Automatic},
RegionFunction->Function[{x,y,z},4*x^2+3*y^2<=12],
VectorStyle->Black,PlotStyle->Opacity[0]];
Show[a,b]
\end{Verbatim}
\end{tcolorbox}

De acuerdo con el ejercicio \ref{ejercicio2paraboloides} de la página \pageref{ejercicio2paraboloides},
la curva de intersección corresponde a la ecuación $4x^{2}+3y^{2}=12$, que representa una elipse.
...
\subsubsection{Flujo sobre el paraboloide \texorpdfstring{$2z=6-2x^2-y^2$}{2z=6-2x^2-y^2}}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
r={x,y,z}={Sqrt[3]*u*Cos[v],2*u*Sin[v],(6-2x^2-y^2)/2};
Integrate[Dot[Cross[D[r,u],D[r,v]],{y^2,x^2,z}],{u,0,1},{v,0,2*Pi}]
\end{Verbatim}
\end{tcolorbox}

\subsubsection{Flujo sobre el paraboloide \texorpdfstring{$z+3=x^2+y^2$}{z+3=x^2+y^2}}
...
Además,
\begin{gather*}
{\bf F}\left({\bm \beta}(u,v)\right) \cdot \left({\bm \beta}_{v}\times {\bm \beta}_{u}\right) = 12u^{4}(2\sin v+\sqrt{3}\cos v)\sin(2v)+\sqrt{3}u(u^{2}(\cos(2v)-7) +6).
\end{gather*}
...
Integrate[Dot[Cross[D[r,v],D[r,u]],{y^2,x^2,z}],{u,0,1},{v,0,2*Pi}]
\end{Verbatim}
\end{tcolorbox}
...
\includegraphics[height=3.5cm]{Img/T_9b.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/T_9c.pdf}
\fuentepropia
\end{figure}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
a=RegionPlot3D[x^2+y^2<4&&x^2+y^2+z^2<=9,{x,-2,2},{y,-2,2},
{z,-3.5,3.5},PlotPoints->90,Mesh->False]
b=SliceVectorPlot3D[{y,-x,z},{x^2+y^2==4,x^2+y^2+z^2==9},{x,-2,2},
{y,-2,2},{z,-3.4,3.5},VectorPoints->16,
RegionFunction->Function[{x,y,z},x^2+y^2<=4&&x^2+y^2+z^2<=9],
VectorStyle->Black,VectorScale->{0.15,Scaled[0.2],Automatic},
PlotStyle->Opacity[0]];
Show[a,b]
\end{Verbatim}
\end{tcolorbox}
...
\textls[-25]{El cálculo anterior se obtiene en \verb"Mathematica" con las siguientes instrucciones:}
...
\end{aligned}
\]
Además, ${\bf F}\left( {\bf w}\left( u,v\right) \right) =\left\langle 2\sin u,-2\cos u,v\right\rangle $ y ${\bf F}\left({\bf w}\left( u,v\right) \right) \cdot \left({\bf w}_{u}\times {\bf w}_{v}\right)= 0$, con lo cual el flujo sobre la superficie del cilindro está dado por:
...
\includegraphics[height=3.5cm]{Img/T_11c.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/T_11d.pdf}
\fuentepropia
\end{figure}

Con las siguientes instrucciones se consigue esta figura.

\begin{SaveVerbatim}{apretar_7}
a=RegionPlot3D[2*x^2+3*y^2<z^2+7&&2*x^2+y^2+z^2<9,
{x,-2,2},{y,-2,2},{z,-3,3},PlotPoints->90,Mesh->False];
b=SliceVectorPlot3D[{z,y,x},{2*x^2+3*y^2==z^2+7,2*x^2+y^2+z^2==9},
{x,-Sqrt[15/4],Sqrt[15/4]},{y,-Sqrt[15/2],Sqrt[15/2]},{z,-4,3},
VectorPoints->20,VectorStyle->Black,RegionFunction->Function[{x,y,z},
x^2+y^2<=4],VectorScale->{0.07,0.2,None},PlotStyle->Opacity[0]];
Show[a,b]
\end{SaveVerbatim}

\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]
\textls[-10]{\UseVerbatim[formatcom=\letraverbatim]{apretar_7}}
\end{tcolorbox}
...
con lo cual
\begin{gather*}
{\bf F}({\bf r}(u,v) ) \cdot ({\bf r}_u \times {\bf r}_v)=
\frac{ 8u \left (u-u \cos (2v) +3 \cos v \, \sqrt{9-2u^2(3+\cos(2v))}\right )}{\sqrt{9-6u^2-2u^2 \cos(2v)}};
\end{gather*}
...
\end{equation}
\textls[-20]{Este cálculo se hizo en \verb"Mathematica" ejecutando las siguientes instrucciones:}

\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
r={x,y,z}={2*u*Cos[v],2*u*Sin[v],Sqrt[9-2*x^2-y^2]};
NIntegrate[2*Dot[{z,y,x},Cross[D[r,u],D[r,v]]],{v,0,2*Pi},{u,0,1}]
\end{Verbatim}
\end{tcolorbox}
...
Las anteriores dos curvas se intersecan en los puntos $(\pm 2, \pm 1)$.
...
\includegraphics[height=3.5cm]{Fig/188.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Fig/189.pdf}
\fuentepropia
\end{figure}
...
R[w_]:={Sqrt[5]*Cos[(b-a)*(1-w)+a],Sqrt[5]*Sin[(b-a)*(1-w)+a]};
r[w_]:={Sqrt[7/2]*Cosh[(d-c)*w+c],Sqrt[7]*Sinh[(d-c)*w+c]};
Animate[ParametricPlot[{R[u],(1-u)*r[t]+u*R[t],r[u]},{u,0,1},
PlotRange->{{-2,3},{-2,3}},PlotStyle->{Red,Black,Red}],{t,0,1}]
\end{Verbatim}
\end{tcolorbox}
...
\includegraphics[height=3cm]{Img/T_11e.pdf} \hspace{5mm}
\includegraphics[height=3cm]{Img/T_11f.pdf}
\fuentepropia
\end{figure}

\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
r={x,y,z}={2*u*Cos[v],2*u*Sin[v],Sqrt[9-2*x^2-y^2]}
A1=ParametricPlot3D[{{x,y,z},{x,y,-z}},{u,0,1},{v,0,2*Pi},
PlotStyle->{RGBColor[0.8,0.2,0.2]},MeshStyle->Black,PlotPoints->90];
{a,b}={ArcCos[2/Sqrt[5]],Pi-ArcCos[2/Sqrt[5]]};
{c,d}={-ArcCosh[2*Sqrt[2/7]],ArcCosh[2*Sqrt[2/7]]};
R=(1-u)*{Sqrt[7/2]*Cosh[(d-c)*t+c],Sqrt[7]*Sinh[(d-c)*t+c]}+
u*{Sqrt[5]*Cos[(b-a)*(1-t)+a],Sqrt[5]*Sin[(b-a)*(1-t)+a]};
{x,z,y}={R[[1]],R[[2]],Sqrt[(z^2+7-2*x^2)/3]};
A2=ParametricPlot3D[{{x,y,z},{x,-y,z},-{x,y,z},-{x,-y,z}},{u,0,1},
{t,0,1},PlotStyle->{Cyan,Orange,Cyan,Orange},PlotPoints->60];
...
4 \iint_H {\bf F}({\bf r}(t,u) ) \cdot ({\bf r}_u \times {\bf r}_v) dtdu \approx 41.0176
\end{equation}
Este valor se obtuvo con \verb"Mathematica" con estas instrucciones:

\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
{a,b,d,c}={ArcCos[2/Sqrt[5]],Pi-a,ArcCosh[2*Sqrt[2/7]],-d};
R=(1-u)*{Sqrt[7/2]*Cosh[(d-c)*t+c],Sqrt[7]*Sinh[(d-c)*t+c]}+
u*{Sqrt[5]*Cos[(b-a)*(1-t)+a],Sqrt[5]*Sin[(b-a)*(1-t)+a]};
r={x,y,z}={R[[1]],Sqrt[(z^2+7-2*x^2)/3],R[[2]]};
NIntegrate[4*Dot[{z,y,x},Cross[D[r,u],D[r,t]]],{u,0,1},{t,0,1}]
\end{Verbatim}
\end{tcolorbox}

El flujo sobre la superficie del sólido es la suma de las expresiones
\ref{ejercicio6e} y \ref{ejercicio6h}. Como ${\rm div}\, {\rm F} =1$, el valor del flujo neto de ${\bf F}$ coincide con el volumen del sólido que se halló en la página \pageref{ejercicio6}, con la expresión \ref{ejercicio6}.
...
\includegraphics[height=3.5cm]{Img/T_10c.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/T_10d.pdf}
\fuentepropia
\end{figure}
...
b=SliceVectorPlot3D[{3*x,y,3*z},{x^2+y^2==3-z,x^2-y^2==3*z+6},
{x,-Sqrt[15/4],Sqrt[15/4]},{y,-Sqrt[15/2],Sqrt[15/2]},
{z,-5,3},RegionFunction->Function[{x,y,z},4*x^2+2*y^2<=15],
VectorPoints->18,VectorStyle->Black,
VectorScale->{0.14,Scaled[0.25],Automatic},PlotStyle->Opacity[0]];
Show[a,b]
\end{Verbatim}
\end{tcolorbox}

En el ejercicio~\ref{ej:ejercicio6hiperbolico} de la página~\pageref{ej:ejercicio6hiperbolico}
se aborda esta región.
La expresión $4x^{2}+2y^{2}=15$ describe la curva de intersección en el plano $xy$. \textls[-10]{Esta región puede representarse mediante las expresiones
$x=\frac{\sqrt{15}}{2}u\cos v$, $y=\sqrt{\frac{15}{2}}u\sin v$, con $u\in \lbrack 0,1]$, $v\in \lbrack 0,2\pi ]$.}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
r={x,y,z}={Sqrt[15]/2*u*Cos[v],Sqrt[15/2]*u*Sin[v],3-x^2-y^2};
Integrate[Dot[Cross[D[r,u],D[r,v]],{3x,y,3z}],{u,0,1},{v,0,2*Pi}]
\end{Verbatim}
\end{tcolorbox}
...
\begin{equation}
\int_{0}^{1}\int_{0}^{2\pi }\left( \frac{75\sqrt{2}}{32}u\left( 5u^{2} + \frac{48}{5}+u^{2}\cos 2v\right) \right) dvdu= \frac{1815}{64}\sqrt{2}\pi. \label{flujo6}
\end{equation}
Con las siguientes instrucciones se hace el cálculo anterior:

\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
r={x,y,z}={Sqrt[15]/2*u*Cos[v],Sqrt[15/2]*u*Sin[v],(x^2-y^2-6)/6};
Integrate[Dot[Cross[D[r,v],D[r,u]],{3x,y,3z}],{u,0,1},{v,0,2*Pi}]
\end{Verbatim}
\end{tcolorbox}
...
sobre la superficie del sólido interior a la esfera con ecuacion $x^2+y^2+z^2=9 $ y al cilindro cardióidico
$r=1+\cos \theta$.
...
\includegraphics[height=3.5cm]{Img/T_12c.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/T_12d.pdf}
\fuentepropia
\end{figure}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
a=RegionPlot3D[(x^2+y^2-x)^2<(x^2+y^2)&&x^2+y^2+z^2<9,{x,-1,2.2},
{y,-2,2},{z,-3,3.2},PlotPoints->90,Mesh->False,BoxRatios->{1,1,1}];
b=SliceVectorPlot3D[{y,-x,z},{(x^2+y^2-x)^2==x^2+y^2,
x^2+y^2+z^2==9},{x,-1,2.2},{y,-2,2},{z,-3,3.2},
RegionFunction->Function[{x,y,z},(x^2+y^2-x)^2<(x^2+y^2)&&
x^2+y^2+z^2<9],VectorPoints->17,VectorStyle->Black,
VectorScale->{0.12,Scaled[0.25],Automatic},PlotStyle->Opacity[0]];
Show[a,b]
\end{Verbatim}
\end{tcolorbox}
...
Por lo tanto, la siguiente función describe la porción sobre la esfera interior al cilindro cardióidico:
\[
{\bf r}(u,\theta) =
\left\langle
...
Por lo tanto, el flujo sobre la superficie de la esfera e interior al cilindro cardióidico está dado por:
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
{x,y,z}={(1+Cos[t])*Cos[t],(1+Cos[t])*Sin[t],Sqrt[9-x^2-y^2]};
...
\includegraphics[height=4cm]{Img/T_14c.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Img/T_14d.pdf}
\fuentepropia
\end{figure}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
a=RegionPlot3D[y>Sqrt[x^2+z^2]&&x^2+y^2+z^2<9,{x,-3,3},{y,0,3},
{z,-3,3},PlotPoints->90,Mesh->False];
b=SliceVectorPlot3D[{x,1,y+z},{x^2+y^2+z^2==9,y==Sqrt[x^2+z^2]},
{x,-3,3},{y,0,3},{z,-3,3},VectorPoints->18,
VectorScale->{0.16,Scaled[0.25],Automatic},VectorStyle->Black,
RegionFunction->Function[{x,y,z},x^2+z^2<=9/2],
PlotStyle->Opacity[0]];
Show[a,b]
\end{Verbatim}
\end{tcolorbox}

\textls[-15]{Las superficies mencionadas se trataron en la página \pageref{ejercicio7a}. Su intersección se caracteriza por la ecuación $x^{2}+z^{2}=\frac{9}{2}$. Por lo tanto, la porción del cono se parametriza como ${\bf r}\left( u,v\right) =\left\langle u\cos v,u,u\sin v\right\rangle$ y la porción de la esfera se parametriza en la forma ${\bf r}\left(u,v\right) =\left\langle u\cos v,\sqrt{9-u^{2}},u\sin v\right\rangle $, en donde $u\in \lbrack 0,\frac{3}{\sqrt{2}}]$, $v\in \lbrack 0,2\pi ]$. Calcularemos el flujo en cada región por separado.}
...
y \ref{flujo10}. Como $\dive{\bf F}=2$, es claro que este valor corresponde a dos veces el volumen del sólido el cual se obtuvo con la expresión \ref{ejercicio7}.
...
\includegraphics[height=4cm]{Img/T_13b.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Img/T_13c.pdf}
\fuentepropia
\end{figure}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
a=RegionPlot3D[(x^2+y^2+z^2-x)^2<x^2+y^2+z^2,{x,-0.5,2.2},
{y,-1.5,1.5},{z,-1.5,1.5},PlotPoints->90,Mesh->False,
BoxRatios->{1,1,1}];
b=SliceVectorPlot3D[{x,y,2*z},{(x^2+y^2+z^2-x)^2==x^2+y^2+z^2},
{x,-0.5,2.2},{y,-1.5,1.5},{z,-1.5,1.5},VectorStyle->Black,
VectorScale->{0.12,Scaled[0.25],Automatic},VectorPoints->15,
PlotStyle->Opacity[0]];
Show[a,b]
\end{Verbatim}
\end{tcolorbox}
...
{\bf r}_{\phi}\times {\bf r}_{\theta} & = \Big\langle\left(\cos\phi+1\right)\left(\sin2\phi+\sin\phi\right)\cos\theta\sin \phi,\\
& \qquad \left( \cos \phi +1\right) \left( \sin 2\phi +\sin \phi \right) \sin \theta \sin \phi ,\\
& \qquad \left( \cos \phi +1\right)^{2}\left( 2\cos \phi -1\right) \sin \phi \Big\rangle .
\end{align*}
Por lo tanto,
\[
...
\]
Como $\dive{\bf F} =4$, es claro que el flujo corresponde a cuatro veces el volumen de la región, el cual se obtuvo en la fórmula \ref{ejercicio5}.
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
a=RegionPlot3D[4*y>x^2+z^2&&x^2+y^2+z^2<12,{x,-3,3},{y,0,4},
{z,-3,3},PlotPoints->90,Mesh->False];
b=SliceVectorPlot3D[{x,y,z},{x^2+y^2+z^2==12,4*y==x^2+z^2},
{x,-3,3},{y,0,3.7},{z,-3,3},
RegionFunction->Function[{x,y,z},x^2+z^2<=8],
VectorPoints->16,PlotStyle->Opacity[0],VectorStyle->Black,
VectorScale->{0.15,Scaled[0.25],Automatic}];
Show[a,b]
\end{Verbatim}
\end{tcolorbox}
...
\includegraphics[height=4cm]{Img/T_15c.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Img/T_15d.pdf}
\fuentepropia
\end{figure}
...
\textls[-10]{Por lo tanto, ${\bf F}({\bf r}(u,v)) = \langle \sqrt{8}u \cos v ,\sqrt{12-8u^2} ,\sqrt{8}u\sin v \rangle$ y ${\bf F}({\bf r}(u,v))\cdot({\bf r}_v \times {\bf r}_u)=\frac{48u}{\sqrt{3-2u^2}}$. El flujo de ${\bf F}$ sobre la esfera está dado por:}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
r={x,y,z}={Sqrt[8]*u*Cos[v],Sqrt[12-8*u^2],Sqrt[8]*u*Sin[v]};
...
Integrate[Dot[{x,y,z},Cross[D[r,u],D[r,v]]],{u,0,1},{v,0,2*Pi}]
\end{Verbatim}
\end{tcolorbox}

El flujo neto sobre la superficie del sólido es la suma de los valores obtenidos en \ref{flujo11}
y \ref{flujo12}. En este caso $\dive{\bf F} =3$, el valor obtenido es tres veces el volumen obtenido en la expresión \ref{ejercicio8}.
...
\includegraphics[height=3.5cm]{Img/T_17d.pdf} \hspace{2mm}
\includegraphics[height=3.5cm]{Img/T_17e.pdf} \hspace{2mm}
\includegraphics[height=3.5cm]{Img/T_17c.pdf}
\fuentepropia
\end{figure}
...
{x,0,4},{y,-3,3},{z,-3,3},PlotPoints->90,Mesh->False];
b=SliceVectorPlot3D[{x-2,z,-y},{x==Sqrt[y^2+z^2],y^2+z^2+4*x==12},
{x,0,3.5},{y,-2,2},{z,-2,2},VectorPoints->18,
RegionFunction->Function[{x,y,z},y^2+z^2<=4],VectorStyle->Black,
VectorScale->{0.16,Scaled[0.24],Automatic},PlotStyle->Opacity[0]];
Show[a,b]
\end{Verbatim}
\end{tcolorbox}
...
en donde $u\in [0,2]$, $v\in \lbrack 0,2\pi ]$.
...
\end{align*}
Entonces, ${\bf F}({\bf r}(u,v)) \cdot ({\bf r}_{u}\times {\bf r}_{v}) = u(2-u)$. Con lo cual, el flujo está dado por:
...
\includegraphics[height=3.5cm]{Img/T_16d.pdf} \hspace{2mm}
\includegraphics[height=3.5cm]{Img/T_16e.pdf} \hspace{2mm}
\includegraphics[height=3.5cm]{Img/T_16f.pdf}
\fuentepropia
\end{figure}
...
\end{equation}
El cálculo anterior se puede hacer en \verb"Mathematica" ejecutando las siguientes instrucciones:

\begin{SaveVerbatim}{apretar_8}
{y,z,x}={Sqrt[15]/2*u*Cos[v],Sqrt[3]*u*Sin[v],y^2+z^2-3};
F=TrigFactor[{y*z,y-x*z,x*y}]
Integrate[Dot[F,Cross[D[{x,y,z},v],D[{x,y,z},u]]],{u,0,1},{v,0,2*Pi}]
\end{SaveVerbatim}

\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]
\textls[-10]{\UseVerbatim[formatcom=\letraverbatim]{apretar_8}}
\end{tcolorbox}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
{y,z,x}={Sqrt[15]/2*u*Cos[v],Sqrt[3]*u*Sin[v],(y^2-z^2+3)/9};
...
\ref{flujo14}, es decir $\frac{5 \sqrt{5}}{16} \pi$. Como $\dive{\bf F}=1$,
...
\includegraphics[height=3.5cm]{Img/T_35a.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/T_35b.pdf}
\end{figure}

\item Considere la superficie sobre el paraboloide  $z=x^2-2x+y^2$ interior al cilindro cardióidico $r=1+\cos t$.

\begin{figure}[H]
\centering
\caption{Superficie sobre un paraboloide}
\includegraphics[height=3.5cm]{Img/T_36a.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/T_36b.pdf}
\end{figure}

Calcule el flujo del campo vectorial ${\bf F}(x,y,z)= \la x-2,y,2z\ra $ sobre la superficie del paraboloide.
$z=x^2-2x+y^2$, interior al cilindro cardióidico $r=1+\cos t$.

\begin{figure}[H]
\centering
\caption[Campo vectorial ${\bf F}(x,y,z)= \la x-2,y,2z\ra $ sobre un paraboloide]{Campo vectorial ${\bf F}(x,y,z)= \la x-2,y,2z\ra $\\
sobre un paraboloide}
\includegraphics[height=3.5cm]{Img/T_36c.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/T_36d.pdf}
\end{figure}

Evalúe
\begin{gather*}
\int_S\rot{\bf F}\cdot d{\bf S}\quad \text{y}\quad \int_C{\bf F}\cdot d{\bf r};
\end{gather*}
en donde $S$ es la superficie en mención y $C$ es su frontera.

\item Calcule el flujo del campo vectorial ${\bf F}(x,y,z)= \la z-y,y,z^2 \ra $ sobre la superficie del paraboloide hiperbólico $9z=x^2-y^2$, interior al cilindro $x^2+y^2 \leq 16$ y exterior al cilindro cardióidico $r=1+\cos t$.

\begin{figure}[H]
\centering
\caption[Campo vectorial ${\bf F}(x,y,z)= \la z-y,y,z^2\ra$ sobre una silla de montar]{Campo vectorial ${\bf F}(x,y,z)= \la z-y,y,z^2\ra$\\ sobre una silla de montar}
\includegraphics[height=3.5cm]{Img/T_37a.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/T_37b.pdf}
\end{figure}
