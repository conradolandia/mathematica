...
Evalúe $\ds \iint_S \rot{\bf F} \cdot d{\bf S}$ en donde ${\bf F}(x,y,z)=\left\langle y^{2},yz-x,xz-y\right\rangle $ y %% $
...
\includegraphics[height=5cm]{Img/53e.pdf} \hspace{5mm}
\includegraphics[height=5cm]{Img/53h.pdf} \\[5mm]
\includegraphics[height=5cm]{Img/53g.pdf}
\fuentepropia
\end{figure}
...
\end{align*}
Además, $\rot{\bf F}\left({\bf r}(u,v)\right)\cdot\left({\bf r}_{u}\times{\bf r}_{v}\right)=-4 u(4u^2\cos(2v)-4u^2+4 u \cos (v)+5)$, entonces,
...
{z,-Sqrt[12-9*x^2]/2,Sqrt[12-9*x^2]/2},{y,4z/9-1,(3+4z-9*x^2-4z^2)/9}]
\end{Verbatim}
\end{tcolorbox}
...
con lo cual el flujo sobre la superficie del cilindro está dado por:
...
\includegraphics[width=0.4\linewidth]{Fig/188.pdf} \hspace{5mm}
\includegraphics[width=0.4\linewidth]{Fig/189.pdf}
\fuentepropia
\end{figure}
...
A2=ParametricPlot3D[{{x,y,z},{x,-y,z},-{x,y,z},-{x,-y,z}},
{u,0,1},{t,0,1},PlotStyle->{Cyan,Orange,Cyan,Orange},PlotPoints->60];
...
RegionFunction->Function[{x,y,z},y^2+z^2<=4],
VectorStyle->Black,VectorScale->{0.16,Scaled[0.24],Automatic},
PlotStyle->Opacity[0]];
Show[a,b]
\end{Verbatim}
\end{tcolorbox}
...
Evalúe $\ds \int_S\rot{\bf F}\cdot d{\bf S}$ y $\ds\int_C{\bf F}\cdot d{\bf r}$; en donde $S$
