...
Las funciones hiperbólicas son un tipo particular de funciones que poseen varias similitudes con las funciones trigonométricas.
Es posible que su nombre corresponda con esa semejanza o al siguiente hecho:
...
Se muestran las figuras de algunas de ellas.

\begin{figure}[H]
  \centering
  \caption{Cono truncado oblicuo determinado por ${\bf r}(u,t)$}
  \includegraphics[height=3.5cm]{Fig/190.pdf} \hspace{2mm}
  \includegraphics[height=3.5cm]{Fig/191.pdf} \hspace{2mm}
  \includegraphics[height=3.5cm]{Fig/192.pdf}
  \fuentepropia
\end{figure}
...
\begin{ejer} Usando la definición de funciones hiperbólicas:
  \begin{enumerate}
    ...
    \item Obtener la figura de las funciones $y=\coth x$, $y=\sech x$, $y=\csch x$ y de las funciones
...
\begin{ejer} Resolver los siguientes problemas:
  \begin{enumerate}
    ...
    \item Muestre que las derivadas de las funciones hiperbólicas inversas son las siguientes, compruebe las siguientes igualdades:
      \begin{itemize}
        \item $\frac{d}{dx}\left(\arcsinh (x)\right) = \dfrac{1}{\sqrt{x^{2}+1}}$
        \item $\frac{d}{dx}\left(\arccosh (x)\right) = \dfrac{1}{\sqrt{x^{2}-1}}$
        \item $\frac{d}{dx}\left(\arctanh (x)\right) = \dfrac{1}{1-x^{2}}$
        \item $\frac{d}{dx}\left(\arccoth (x)\right) = \dfrac{1}{1-x^{2}}$
        \item $\frac{d}{dx}\left(\arcsech (x)\right) = \dfrac{-1}{x\sqrt{1-x^{2}}}$
        \item $\frac{d}{dx}\left(\arccsch (x)\right) = \dfrac{-1}{x\sqrt{1+x^{2}}}$
      \end{itemize}
  \end{enumerate}
\end{ejer}
...
\begin{figure}[H]
  ...
  \includegraphics[height=3.5cm]{Fig/193.pdf} \hspace{5mm}
  \includegraphics[height=3.5cm]{Fig/194.pdf}
  \fuentepropia
\end{figure}

\begin{teorema}[\bfseries Teorema de Pappus para volúmenes] \index{Teorema de Pappus!volumen}
  \textls[-5]{Si una región plana girar alrededor de una recta en el plano, tal que la recta no corta el interior de la~región, entonces el volumen $V$ del sólido generado es el área $A$ de la región por~la distancia $2\pi r$ recorrida por su centroide durante la rotación. Si $r$ es la distancia entre el eje de rotación y el centroide, es decir $V=2\pi rA$.}
\end{teorema}
\begin{proof}
  ...
  \item Con las mismas consideraciones anteriores, si la región $R$ gira alrededor de la recta $y=c$,
    ...
    por lo tanto, $\mu_x= \rho\overline{y} A$. El momento con respecto a la recta $L$ con ecuación $y=c$ está dado por:
    \begin{align*}
    ...
    \begin{align*}\mu_{L} &= \frac{1}{2}\rho\int_{a}^{b}\left([f(x)]^2-[g(x)]^2\right)dx-c\rho\int_{a}^{b}\left(f(x) -g(x)\right)dx. \\
    &= \mu_x-c\rho A.
    \end{align*}
...
así, es posible establecer una aproximación a la longitud de arco.
\begin{align*}
...
\begin{figure}[H]
  ...
  \includegraphics[height=3.5cm]{Fig/195.pdf} \hspace{5mm}
  \includegraphics[height=3.5cm]{Fig/196.pdf}
  \fuentepropia
\end{figure}
...
El volumen del sólido generado por la rotación del $i-$ésimo rectángulo está dado por:
\begin{eqnarray*}
...
\begin{align*}
  V &= \lim_{n\rightarrow \infty }\left( \sum_{i=1}^{n}\pi \left(g\left( t_{i}^{\ast }\right) \right) ^{2}\left( \frac{f\left( t_{i}+\Delta t\right) -f\left( t_{i}\right) }{\Delta t}\right) \Delta t\right)\\
  &= \int_{a}^{b}\pi \left[ g\left( t\right) \right] ^{2}f^{\prime }(t) dt.
\end{align*}
\endgroup
...
\begin{figure}[H]
  ...
  \includegraphics[height=3.5cm]{Fig/197.pdf} \hspace{5mm}
  \includegraphics[height=3.5cm]{Fig/198.pdf}
  \fuentepropia
\end{figure}
...
\includegraphics[height=4cm]{Fig/199.pdf}
\fuentepropia
\end{figure}
...
Hallar el área de la superficie generada y el volumen del sólido acotado por esta región.

En el siguiente enlace, \href{https://www.math3d.org/zbgohcryUD}{https://www.math3d.org/zbgohcryUD}, puede encontrarse esta construcción.

\begin{figure}[H]
\centering
\caption{Superficie de revolución}
\includegraphics[height=3cm]{Img/T_74a.pdf} \hspace{2mm}
\includegraphics[height=3cm]{Img/T_74b.pdf} \hspace{2mm}
\includegraphics[height=3cm]{Img/T_74c.pdf}
\fuentepropia
\end{figure}
...
\section[Coordenadas curvilíneas ortogonales]{Coordenadas curvilíneas ortogonales}
...
cuya transformación inversa existe: $u=u(x,y,z)$, $v=v(x,y,z)$, $w = w(x,y,z)$.
...
El sistema se denomina \emph{ortogonal} si las tangentes a las curvas coordenadas en todo punto de intersección son ortogonales. Es decir, las coordenadas $u,v,w$ forman un sistema de \emph{coordenadas curvilíneas ortogonales} si el conjunto
...
\textls[-10]{Una exposición más profunda de estos temas puede consultarse en~\cite{poljanin_handbook_2007}.}
...
\centering
\caption{Plano polar}
\includegraphics[height=4cm]{Fig/200.pdf}
\fuentepropia
\end{figure}
...
\includegraphics[height=4cm]{Fig/201.pdf}
\fuentepropia
\end{figure}

\begin{prop}
En coordenadas bipolares con valores constantes de $u$ y $v$ representan círculos en el plano $xy$.
\end{prop}
...
Para la primera de estas expresiones es:
\begin{align}
\label{bipolares1}
...
&=  \frac{a^2\left( \cosh v-\cos u\right) ^2 }{\left( \cosh v-\cos u\right) ^2\sin ^2u} = a^2{\rm csch}^2u
\end{align}
...
\includegraphics[height=3.8cm]{Fig/202.pdf}
\fuentepropia
\end{figure}
...
\subsubsection{Coordenadas cilíndricas elípticas}\index{Coordenadas!cilíndricas elípticas}
...
\includegraphics[height=3.5cm]{Img/T_63a.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/T_63b.pdf}
\fuentepropia
\end{figure}
...
\subsubsection{Coordenadas cilíndricas parabólicas}  \index{Coordenadas!cilíndricas parabólicas}
La relación con las coordenadas cartesianas y el sistema de coordenadas cilíndricas parabólicas son las siguientes:
\begin{equation}
...
\begin{figure}[H]
\centering
\caption{Coordenadas cilíndricas parabólicas}
\includegraphics[height=4cm]{Fig/203.pdf}
\fuentepropia
\end{figure}
...
\centering
\includegraphics[height=3cm]{Img/T_66a.pdf} \hspace{5mm}
\includegraphics[height=3cm]{Img/T_66b.pdf}
\fuentepropia
\end{figure}
...
\includegraphics[height=3.5cm]{Img/T_64a.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/T_64b.pdf}
\fuentepropia
\end{figure}
...
u=Pi/6;
A1=ParametricPlot3D[{x,y,z},{v,-4,4},{z,-2,2},MeshStyle->Black,
PlotStyle->Orange,Mesh->10,PlotPoints->50];
v=1/2;
A2=ParametricPlot3D[{x,y,z},{u,0,2*Pi},{z,-2,2},MeshStyle->Gray,
PlotPoints->50,PlotStyle->Cyan];
z=1;
A3=ParametricPlot3D[{x,y,z},{u,0,2*Pi},{v,-2,2},MeshStyle->Gray,
PlotPoints->50,PlotStyle->Orange];
Show[A1,A2,A3,PlotRange->{{-3,4},{-3,4},{-2,2}}]
\end{Verbatim}
\end{tcolorbox}

El resultado gráfico es el que se observa en la figura~\ref{fig:cilindricas_bipolares}.

\begin{figure}[H]
\centering
\caption{Superficies constantes en coordenadas cilíndricas bipolares}
\label{fig:cilindricas_bipolares}
\includegraphics[height=3.5cm]{Img/T_65a.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/T_65b.pdf}
\fuentepropia
\end{figure}
...
\begin{align*}
\tan u &= \frac{2ay}{x^{2}+y^{2}-a^{2}}, \\
\tanh v &= \frac{2ax}{x^{2}+y^{2}+a^{2}}, \\
...
\begin{figure}[H]
\centering
\caption{Construcción de un toro}
\includegraphics[height=3cm]{Fig/204.pdf} \hspace{2mm}
\includegraphics[height=3cm]{Fig/205.pdf}
\fuentepropia
\end{figure}
...
\textls[-10]{Las superficies relacionadas con valores constantes de $v$ corresponden a \emph{toros con su corazón circular de radio $a$ en el plano ecuatorial}, cuando $u$ es constante se generan esferas ortogonales a los toros cuyo centro está en el eje polar, y con $\phi$ constante se obtienen semiplanos. Las siguientes ecuaciones determinan la relación entre coordenadas cartesianas y coordenadas toroidales.}
\begin{equation}
x = \frac{a\sinh v\cos \phi }{\cosh v-\cos u},\quad y =\frac{a\sinh v\sin \phi }{\cosh v-\cos u}, \quad z = \frac{a\sin u}{\cosh v-\cos u}
\end{equation}
...
Lo cual implica que $\sqrt{x^2+y^2}=\dfrac{a\sinh v}{\cosh v-\cos u}$.

Otra equivalencia es:
\begin{eqnarray}
x^2+y^2+z^2+a^2 &=& \frac{a^2 \sinh ^2v}{\left(\cosh v-\cos u\right) ^2}
+\left( \frac{a\sin u}{\cosh v-\cos u}\right) ^2+a^2 \nonumber \\
&=& \frac{2a^2 (\cosh ^{2}v-\cosh v\cos u)}{\cosh v-\cos u} \\
&=& \frac{2a^2 \cosh v}{\cosh v-\cos u} \label{toroidales2}
\end{eqnarray}
...
\begin{align*}
x^{2}+y^{2} &=\left( \frac{a\sinh v\cos \phi }{\cosh v-\cos u}\right)^{2}
+\left( \frac{a\sinh v\sin \phi }{\cosh v-\cos u}\right)^{2} \\
&= \frac{a^{2}\sinh ^{2}v}{\left( \cos u-\cosh v\right) ^{2}} \\
\left( z-a\cot u\right) ^{2} &= \left( \frac{a\sin u}{\cosh v-\cos u}-\frac{a\cos u}{\sin u}\right) ^{2} \\
&= \frac{a^{2}\left( 1-\cos u\cosh v\right) ^{2}}{\left(\left( \cosh v-\cos u\right) \sin u\right) ^{2}}
\end{align*}
Se sigue que
\begin{gather}
...
\end{gather}
La ecuación \ref{toroidales4} representa una esfera para valores constantes de $u$.
\end{proof}
...
Su visualización gráfica es la siguiente.

\begin{figure}[H]
\centering
\caption{Superficies constantes en coordenadas toroidales}
\label{fig:toroidales_constantes}
\includegraphics[height=3.5cm]{Img/T_57a.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/T_57b.pdf}
\fuentepropia
\end{figure}
...
Las coordenadas esferoides oblatas (achatadas) se originan cuando una elipse rota alrededor de su eje menor, la relación con las coordenadas cartesianas es la siguiente.

{\setlength{\abovedisplayskip}{0pt}
\setlength{\abovedisplayshortskip}{0pt}
\begin{gather}
x = a \cosh\eta\cos\theta\cos\phi, y = a \cosh\eta\cos\theta\sin\phi, z= a\sinh\eta\sin\theta
\end{gather}}

Con $\eta \in \mathbb{R}$, $0\leq \theta <2\pi$ y $0\leq \phi <\pi$.
Con la definición se pueden demostrar las siguientes identidades:
\begin{align*}
\frac{x^{2}+y^{2}}{a^{2}\cosh ^{2}\eta }+\frac{z^{2}}{a^{2}\sinh ^{2}\eta} &=  \frac{a^{2}\cos ^{2}\theta \cosh ^{2}\eta }{a^{2}\cosh ^{2}\eta }+\frac{a^{2}\sinh ^{2}\eta \sin ^{2}\theta}{a^{2}\sinh ^{2}\eta }= 1 \\
...
\includegraphics[height=4cm]{Img/T_58a.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Img/T_58b.pdf}
\fuentepropia
\end{figure}
...
\[Eta]=3/2;
A1=ParametricPlot3D[{x,y,z},{\[Theta],0,2*Pi},{\[Phi],0,Pi}];
\[Theta]=Pi/6;
A2=ParametricPlot3D[{x,y,z},{\[Eta],-2,2},{\[Phi],0,2*Pi},
PlotStyle->Orange];
\[Phi]=Pi/4;
...
\begin{gather}
x = a\sinh \eta \sin \theta \cos \phi, y = a\sinh \eta \sin \theta \sin \phi, z=a\cosh \eta \cos \theta
\end{gather}
...
\includegraphics[height=4cm]{Img/T_59a.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Img/T_59b.pdf}
\fuentepropia
\end{figure}
...
\begin{prop}
En coordenadas cónicas con valores constante de $v$ se representan conos elípticos
con eje de simetría el eje $z$.
\end{prop}
...
describe un cono elíptico con $x$ como eje de simetría y $w$ constante.
...
\begin{align}
\hskip-0.3cm x^2+y^2+z^2 &= \frac{a^2(\left(\sin\theta \cos \phi\right)^2+\left( \sin \theta \sin \phi \right) ^2+\left( \sinh \eta \right) ^2)}{\left( \cosh \eta -\cos \theta \right)^2}\nonumber \\
...
para $\theta >\pi/2$ se produce una superficie en forma de \emph{limón}.

\begin{figure}[H]
\centering
\caption{Superficies constantes en coordenadas\\ esferoidales prolatas}
\includegraphics[height=4cm]{Img/T_60a.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Img/T_60b.pdf}
\fuentepropia
\end{figure}
Con las siguientes instrucciones se obtienen estas figuras:
...
{x,y,z}={Sin[\[Theta]]*Cos[\[Phi]]/w,Sin[\[Theta]]*Sin[\[Phi]]/w,
Sin[\[Eta]]/w}\[Phi]=Pi/3;
...
\subsubsection{Coordenadas cardióidicas} \index{Coordenadas!cardióidicas}
...
\centering
\caption{Superficies constantes en coordenadas\\ esferoidales prolatas}
\label{fig:superficies_constantes_esferoidales_prolatas}
\includegraphics[height=3.5cm]{Img/T_61a.pdf}\hskip0.5cm
\includegraphics[height=3.5cm]{Img/T_61b.pdf}
\fuentepropia
\end{figure}
...
La figura~\ref{fig:superficies_constantes_esferoidales_prolatas} se obtiene utilizando las siguientes instrucciones:
...
\begin{prop}
En coordenadas cardióidicas, para valores constantes de $u$ y $v$
...
\begin{gather*}
x^2+y^2+z^2 = \left( \frac{uv\cos \theta}{\left( u^2+v^2\right) ^2} \right) ^2+\left( \frac{uv\sin \theta}{\left( u^2+v^2\right) ^2} \right) ^2+\left( \frac{1}{2}\frac{u^2-v^2}{\left(u^2+v^2\right) ^2}\right) ^2 = \frac{1}{4\left(u^2+v^2\right) ^2}.
\end{gather*}
...
De la ecuación \ref{elipsoidales1} se observa que si $u$ es constante la superficie obtenida es un elipsoide. Con $v$ constante, la ecuación \ref{elipsoidales2} corresponde a un \mbox{hiperboloide} de un manto y si $w$ es constante entonces la ecuación \ref{elipsoidales3} determina un hiperboloide de dos mantos.

\begin{ejer}
Hallar algunas superficies constantes en coordenadas elipsoidales.
\end{ejer}

\subsubsection{Coordenadas esféricas y cilíndricas}\index{Coordenadas!esféricas} \index{Coordenadas!cilíndricas}
...
permite concluir que $\phi $ es constante de expresiones que determinan semiconos.
...
z=\[Rho]*Cos[\[Phi]];
\[Phi]=Pi/3;
A1=ParametricPlot3D[{x,y,z},{\[Theta],0,2*Pi},{\[Rho],0,3}];
\[Theta]=Pi/6;
A2=ParametricPlot3D[{x,y,z},{\[Phi],0,Pi},{\[Rho],0,3}];
\[Rho]=2;
...
El producto obtenido se puede ver en la figura~\ref{fig:esfericas}.

\begin{figure}[H]
\centering
\caption{Superficies constantes en coordenadas esféricas}
\label{fig:esfericas}
\includegraphics[height=3.5cm]{Img/T_62a.pdf} \hspace{2mm}
\includegraphics[height=3.5cm]{Img/T_62b.pdf} \hspace{2mm}
\includegraphics[height=3.5cm]{Img/T_62c.pdf}
\fuentepropia
\end{figure}

En coordenadas cilíndricas se establece que
