\chapter{Elementos básicos en Mathematica}
...
\begin{tcolorbox}[title=Advertencia]
...
{\small
  \verb"ArcSin[x]," \verb"ArcCos[x],"
  \verb"ArcTan[x]," \verb"ArcCot[x],"
  \verb"ArcSec[x]," \verb"ArcCsc[x]"
}\!.
...
\begin{tcolorbox}[title=Algunas funciones básicas en Wolfram]
  \begin{tabular}{rl}
...
\begin{tcolorbox}[title=Funciones básicas]
...
\begin{tcolorbox}[title=Usos de \verb"ExpandDenominator" y \verb"ExpandNumerator"]
\begin{Verbatim}[formatcom=\letraverbatim]
...
\subsection{Tablas}
\textls[-10]{Para generar una lista con $n$ términos de la expresión $f(k)$ se usa la expresión \verb"Table[f[k],n]", además con \verb"Table[f[k],{k,a,b}]" se consigue que la lista satisfaga que $a \leq k \leq b$. Ilustremos esto con dos ejemplos sencillos.}\index{Table}
...
\begin{tcolorbox}
  \verb"Table[k^2,{k,2,20}"\\
    ...
    \textls[-10]{El vector ${\bf u}= \langle a,b,c \rangle$ se define en \verb"Mathematica" como una lista \verb"u={a,b,c}".}
    El producto punto, el producto cruz y la norma de los vectores ${\bf u}$ y ${\bf v}$ se determina como sigue.
    \begin{tcolorbox}[title=Vectores]
      \verb"Dot[u,v]" \hskip1.75cm \verb"Cross[u,v]" \hskip1.75cm \verb"Norm[u]"
    \end{tcolorbox}

    \begin{tcolorbox}[title=Funciones vectoriales]
    ...
    \textls[-10]{Se pueden hacer algunos procedimientos asumiendo ciertas condiciones. Las funciones
      \verb"Assuming"\index{Assuming} y \verb"Assumptions"\index{Assumptions}, permiten este tratamiento.
    En los siguientes ejemplos se evidencia la diferencia de usar estas posibilidades.}

    \begin{tcolorbox}[title=Usos de \verb"Assuming" y \verb"Assumptions"]
\begin{Verbatim}[formatcom=\letraverbatim]
Norm[{Cos[t],Sin[t],t}]
...
\end{Verbatim}
    ...
    \subsection{Sumas}
    \textls[-15]{El comando \verb"Sum[Expresión,{k,a,b}]" obtiene la suma de una \verb"Expresión"} con variable de sumación $k$, la cual inicia en $a$ y termina en $b$. Ejemplo de lo anterior son las siguientes sumas.
    ...
    \begin{tcolorbox}[title=Funciones \verb"Mathematica"]
      \begin{multicols}{2} % Activa las dos columnas
\begin{Verbatim}[formatcom=\letraverbatim]
Sum[k^3,{k,1,n}]
Sum[k^2,{k,1,10}]
...
\end{Verbatim}
    ...
    \begin{equation*}
    \lim_{x\to0}\frac{\sin(3x)}{\sin(2x)}\hskip1.5cm
...
\end{Verbatim}
    ...
    \begin{itemize}
      \item Correcta {\small \verb"Limit[n/(n+m),{n,m}->{Infinity,Infinity}]"}
      \item Incorrecta {\small \verb"Limit[Limit[n/(n+m),m->Infinity],n->Infinity]"}
      \item Incorrecta {\small \verb"Limit[Limit[n/(n+m),n->Infinity],m->Infinity]"}
    \end{itemize}

    \subsection{Operadores básicos}
    %\begin{tcolorbox}[title=Operadores Básicos en \verb"Mathematica"]
    ...
    \begin{itemize}
      ...
      \item La derivada parcial de $f(x,y,z)$ con respecto a $x$, se consigue ejecutando \verb"D[f[x,y,z],x]", mientras que
        ...
        \verb"D[x^y,x]". \index{Diferencial total}
      \item \textls[-15]{La integral indefinida de la función $f(t)$ con respecto a $t$,
...
\end{Verbatim}
    ...
    \begin{tcolorbox}[title=Las funciones {\tt Manipulate} y {\tt Animate} ]
...
\end{Verbatim}
    ...
    \begin{tcolorbox}[title=Ejemplos con {\tt Manipulate} y {\tt Animate}]
\begin{Verbatim}
...
\end{Verbatim}
    ...
    \begin{tcolorbox}[title=Definición de funciones]
    ...
    \begin{tcolorbox}[title=Definición de funciones a trozos]
    ...
    \begin{itemize}
      ...
      \item La función
        $$ f(x,y) = \left \{
          \begin{array}{cl} \frac{xy}{x^2-y^2} & \text{ si } x>y \\[0.2cm] \sin(x+y) & \text{en caso contrario}
          \end{array} \right.$$
        ...
        \begin{itemize}
          ...
          \item \verb"ContourPlot[f[x,y]==0,{x,a,b},{y,c,d}]" dibuja la ecuación de dos variables $f(x,y)=0$, con $x \in[a,b]$ y $y \in[c,d]$, se debe incluir el símbolo de igualdad y no olvidar “$==$”.\index{ContourPlot}
          \item \verb"ParametricPlot[{x[t],y[t]},{t,a,b}]" dibuja la curva parametrizada $\ve{r}(t)=\left\langle x(t),y(t)\right\rangle ,\,\, t\in \lbrack a,b]$.
      ...
      \begin{tcolorbox}[title=Algunos gráficos en el plano]
\begin{Verbatim}
...
\end{Verbatim}
      ...
      \begin{itemize}
        ...
        \item \textls[-30]{\verb"ParametricPlot3D[\{x[u,v],y[u,v],z[u,v]\},\{u,a,b\},\{v,c,d\}]"}
          se utiliza para graficar superficies parametrizadas por medio de una función
          $\ve{r}(u,v)=\langle x(u,v),y(u,v),z(u,v) \rangle $, con
          $u\in[a,b]$, $v\in[c,d]$.
        \item \verb"ContourPlot3D[f[x,y,z]==0,{x,a,b},{y,c,d},{z,p,q}]" se \linebreak
          usa para graficar conjuntos de nivel (contornos).
      ...
      \begin{tcolorbox}[title=Opciones de los gráficos]
        \begin{tabular}{rl}
      ...
      \begin{itemize}
        \item \verb"Plot3D[{5-y^2,x^2+y^2},{x,-4,4},{y,-4,4},"\\
          \verb"PlotPoints->60,PlotStyle->Opacity[0.6]]"
        \item \verb"ContourPlot3D[{z==6-(x-1)^2-y^2,4*x^2+y^2==36,"\\
            \verb"x^2+4*y^2==4},{x,-6,10},{y,-7,7},{z,-35,6},"\\
          \verb"ContourStyle->{Red,Yellow,Green}]"
        \item \verb"ParametricPlot3D[{Cos[u]*Cosh[v],Sin[u]*Cosh[v],"\\
            \verb"Sinh[v]},{u,0,2*Pi},{v,-2,2},PlotStyle->Orange,"\\
          \verb"MeshStyle->Gray]"
        \item \verb"ContourPlot3D[z==3*x-x^3-2*y^2+y^4,{x,-3,3},{y,-2,3},"\\
          \verb"{z,-3,3},MeshStyle->Gray]"
\item \begin{Verbatim}
ContourPlot3D[z==3*x-x^3-2*y^2+y^4,{x,-3,3},
{y,-2,3},{z,-3,3},ColorFunction->Function[{x,y,z},
ColorData["Rainbow"][z]],MeshStyle->Gray]
\end{Verbatim}
      \end{itemize}

      La siguiente figura se consigue utilizando las instrucciones señaladas.
      ...
      \begin{tcolorbox}[title=Instrucción en \verb"Mathematica"]
...
\end{Verbatim}
      ...
      \begin{tcolorbox}[title=Uso de la función \verb"Show"]
\begin{Verbatim}
...
\end{Verbatim}
      ...
      \begin{tcolorbox}[title=Ejecución de gráficos por separado]
\begin{Verbatim}
...
\end{Verbatim}
      ...
      \begin{tcolorbox}[title=Gráfico de puntos]
      ...
      \begin{tcolorbox}[title=Gráfico de segmentos]
\begin{Verbatim}
...
\end{Verbatim}
      ...
      \begin{tcolorbox}[title=Gráfico de vectores en el espacio]
\begin{Verbatim}
...
\begin{figure}[t]
  \begin{center}
    ...
    \includegraphics[height=4.5cm]{Img/T_67a.pdf} \hskip5mm
...
\begin{figure}[H]
  \centering
  \caption{Uso de la función {\tt RegionPlot}}
  \includegraphics[height=2.4cm]{Img/T_49a.pdf} \hspace{3mm}
  \includegraphics[height=2.4cm]{Img/T_49b.pdf} \hspace{3mm}
  \includegraphics[height=2.4cm]{Img/T_49c.pdf} \hspace{3mm}
  \includegraphics[height=2.4cm]{Img/T_49d.pdf}
  \fuentepropia
\end{figure}
...
\begin{tcolorbox}
  \begin{Verbatim}[formatcom=\letraverbatim]
  RegionPlot[1<=x^2+2*y^2<=9&&y>=-x^2,{x,-3,3},{y,-2,5/2},
  PlotPoints->50,BoundaryStyle->{Black,Thickness->0.01},
  PlotStyle->Gray,Frame->False]
  RegionPlot[(x+1)^2+2*y^2<3||(x-1)^2+y^2<3,{x,-3,3},{y,-2,2},
  BoundaryStyle->{Black,Thickness->0.01},PlotStyle->Gray,Frame->False]
  RegionPlot[{(x+1)^2+2*y^2<3,(x-1)^2+y^2<3},{x,-3,3},{y,2,2},
  PlotStyle->Gray,BoundaryStyle->{Black,Thickness->0.01},Frame->False]
  RegionPlot[x^2+y^2<9&&Abs[x]+Abs[y]>2,{x,-3,3},{y,-3,3},
  Frame->False,PlotStyle->Gray,BoundaryStyle->{Black,Thickness->0.01}]
  \end{Verbatim}
\end{tcolorbox}
...
Este comando dibuja la región a partir de ciertas \verb"DESIGUALDADES", con $x\in[a,b]$, $y\in[c,d]$, $z\in[u,v]$.
Las inecuaciones se concatenan con los símbolos \verb"&&".
La sintaxis básica es:

\begin{tcolorbox}[title=Gráfico de una región sólida]
\begin{Verbatim}[formatcom=\letraverbatim]
...
\end{Verbatim}
    ...
    1)\enspace El sólido acotado por las figuras de $z=0$, $z=1-y$, $x^2=y$, $y=1$.

    \begin{figure}[H]
      \centering
      \caption{Cono truncado oblicuo determinado por ${\bf r}(u,t)$}
      \includegraphics[height=4cm]{Img/math_06c.pdf} \hspace{5mm}
      \includegraphics[height=4cm]{Img/math_06d.pdf}
      \fuentepropia
    \end{figure}
...
\end{Verbatim}
    ...
    %\clearpage
  2)\enspace El sólido acotado por las figuras de $(x-1)^2+y^2= 1$, $(x-3/2)^2+y^2=4$, $z=0$, $2z+y=4$.

  \begin{figure}[H]
    \centering
    \caption{Sólido acotado por $x^2+2x+y^2=0$, $x^2+y^2-3x=\frac{7}{4}$, $2z+y=4$}
    \includegraphics[height=3.5cm]{Img/math_06.pdf} \hspace{5mm}
    \includegraphics[height=3.5cm]{Img/math_06a.pdf} \hspace{5mm}
    \includegraphics[height=3.5cm]{Img/math_06b.pdf}
    \fuentepropia
  \end{figure}
  ...
  \begin{tcolorbox}
\begin{Verbatim}[formatcom=\letraverbatim]
RegionPlot3D[z>0&&z<4-y/2&&(x-1)^2+y^2>1&&(x-1.5)^2+y^2<4,
{x,-1,4},{y,-2,2},{z,0,5},Mesh->False,PlotPoints->90]
\end{Verbatim}
  \end{tcolorbox}

3)\enspace Las expresiones $(x^2+y^2)^2=z+1$, $z= 4(1-x^2-y^2)$
...
4)\enspace La región del espacio interior a la esfera $x^2+y^2+z^2=4$ y al hiperboloide de dos mantos $x^2+y^2=2+z^2$, exterior al cilindro $x^2+y^2=\frac{2}{5}$ se muestra en la siguiente figura.

\begin{figure}[H]
\centering
\caption{Sólido acotado por $x^2+y^2+z^2=4$, $x^2+y^2=2+z^2$, $x^2+y^2=\frac{2}{5}$}
\includegraphics[height=4.5cm]{Img/math_06n.pdf} \hspace{5mm}
\includegraphics[height=4.5cm]{Img/math_06p.pdf}
\fuentepropia
\end{figure}
...
5)\enspace En coordenadas cilíndricas, las desigualdades
$1 \leq r \leq 2,\,\, r \leq z \leq 2, $
determinan la región acotada por un cilindro, un semicono y un plano.

\begin{figure}[H]
\centering
\caption{Sólido acotado por $1 \leq r \leq 2$, $r \leq z \leq 2$}
\includegraphics[height=4.5cm]{Img/math_06g.pdf} \hspace{5mm}
\includegraphics[height=4.5cm]{Img/math_06h.pdf}
\end{figure}
...
\begin{tcolorbox}[title=Gráficos en coordenadas esféricas]
\begin{Verbatim}[formatcom=\letraverbatim]
SphericalPlot3D[f[\[Theta],\[Phi]],{\[Theta],a,b},{\[Phi],c,d}]
\end{Verbatim}
\end{tcolorbox}
...
\begin{tcolorbox}
\begin{Verbatim}[formatcom=\letraverbatim]
SphericalPlot3D[1,{\[Theta],Pi/6,Pi},{\[Phi],Pi/6,5*Pi/3}]
SphericalPlot3D[Cos[\[Phi]],{\[Theta],0,Pi},{\[Phi],0,Pi}]
SphericalPlot3D[\[Theta],{\[Theta],0,Pi},{\[Phi],0,2*Pi}]
\end{Verbatim}
\end{tcolorbox}

Es así como se obtienen, respectivamente, los resultados de la figura~\ref{regiones_esfe}.

\begin{figure}[t]
\centering
\caption{Regiones en coordenadas esféricas}
\label{regiones_esfe}
\includegraphics[height=4cm]{Img/math_06aa.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Img/math_06ab.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Img/math_06ac.pdf}
\fuentepropia
\end{figure}
...
y luego establecer la región que se ilustra en la figura~\ref{fig:regiones_esfe_2}.

\begin{figure}[H]
\centering
\caption{Región determinada por $1\leq \rho \leq 2$, $0\leq \theta\leq \frac{3\pi}{4}$ y $\frac{\pi}{6} \leq \phi \leq \frac{3\pi}{4}$}
\label{fig:regiones_esfe_2}
\includegraphics[height=4cm]{Img/math_06m.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Img/math_06l.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Img/math_06k.pdf}
\fuentepropia
\end{figure}

Para conseguirlo, se pueden definir las variables $\rho$, $\theta$, $\phi$ y luego establecer la región que se ilustra a continuación:

\begin{tcolorbox}[title=Regiones en el espacio]
\begin{Verbatim}[formatcom=\letraverbatim]
...
Utilizando la función \verb"ArcTan[x,y]", se puede obtener la región del espacio determinada por las expresiones $1\leq \rho  \leq 2$, $0\leq \theta \leq 3\pi/2$, $\pi/4\leq \phi \leq 5\pi/6$.

\begin{figure}[H]
\centering
\caption{Región determinada por $1\leq\rho\leq 2$, $0\leq\theta\leq 3\pi/2$, $\pi/4\leq\phi\leq 5\pi/6$}
\includegraphics[height=5cm]{Img/T_50a.pdf} \hspace{5mm}
\includegraphics[height=5cm]{Img/T_50b.pdf}
\fuentepropia
\end{figure}
...
\begin{tcolorbox}[title=Instrucciones en Mathematica]
\begin{Verbatim}[formatcom=\letraverbatim]
...
La función \verb"ToSphericalCoordinates[{x,y,z}]" permite transformar coordenadas rectangulares en esféricas, mientras que la transformación inversa se logra con \textls[-30]{\Verb!FromSphericalCoordinates[{\[Rho],\[Theta],\[Phi]}]!}.
...
dentro de las opciones de un gráfico se pueden agregar restricciones a este. \index{RegionFunction} Sobre la esfera $x^2+y^2+z^2=9$ se aplica la restricción $-2yz<x^2+z^2<9$ y el paraboloide $z=5-x^2-y^2$ con la condición $2x<x^2+y^2<9$.

\begin{figure}[H]
\centering
\caption{Superficies con restricciones en el espacio}
\includegraphics[height=4cm]{Img/T_47a.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Img/T_48a.pdf}
\fuentepropia
\end{figure}
...
\begin{tcolorbox}[title=Uso del comando {\tt RegionFunction} ]
\begin{Verbatim}[formatcom=\letraverbatim]
...
\includegraphics[height=3.5cm]{Img/T_44a.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/T_44b.pdf} \\[5mm]
\includegraphics[height=3.5cm]{Img/T_44c.pdf}
\fuentepropia
\end{figure}
...
obtenida mediante el uso de la siguiente instrucción.
...
$0.2$ respectivamente, se obtiene la figura~\ref{fig:funcion_T45}.

\begin{figure}[H]
\centering
\caption{Función $f(x,y)=3+xy-x^2-y^3$}
\label{fig:funcion_T45}
\includegraphics[height=3.5cm]{Img/T_45a.pdf} \hspace{5mm}
\includegraphics[height=3.5cm]{Img/T_45b.pdf}
\fuentepropia
\end{figure}

Las sentencias para obtener la figura~\ref{fig:funcion_T45} son las siguientes.
...
La figura anterior se consigue al ejecutar las siguientes instrucciones. La condición se obtiene al usar la opción \verb"RegionFunction". \index{RegionFunction}
...
\begin{tcolorbox}[title=Sólidos de revolución]
...
\begin{tcolorbox}[title=Sólido de revolución con una curva parametrizada]
...
\begin{figure}[H]
\centering
\caption{Superficie generada por ${\bf r}(t)= \langle 1 - \cos(t), \sin(t), t - t^2\rangle$}
...
\begin{tcolorbox}[title=Ajuste de puntos]
\begin{Verbatim}[formatcom=\letraverbatim]
...
Fit[{{0,Pi/2,2},{Pi/3,Pi/2,-1},{2*Pi/3,Pi,-2},{-Pi,2*Pi,1}},
{Sin[x],Cos[y],Cos[x*y]},{x,y}]
Fit[{{1,1,2,2},{1,2,2,0},{-1,-2,3,4},{-3,1,2,-1},{-2,4,0,1}},
{x^2*y*z,x*y^2,y*z^2,Sin[x+y],Cos[y+z]},{x,y,z}]
\end{Verbatim}
\end{tcolorbox}
...
\begin{tcolorbox}[title=Polinomio de interpolación]
\begin{Verbatim}[formatcom=\letraverbatim]
...
\begin{tcolorbox}[title=Integrales dobles]
\begin{Verbatim}[formatcom=\letraverbatim]
...
\begin{tcolorbox}[title=Integrales triples]
\begin{Verbatim}[formatcom=\letraverbatim]
...
%\begin{tcolorbox}  \begin{Verbatim}[fontsize=\letraverbatim]
...
\section{Ecuaciones diferenciales con DSolve}
...
\index{Ecuaciones diferenciales}
El comando \verb"DSolve" \index{DSolve} resuelve una o varias ecuaciones diferenciales ordinarias o parciales.
...
\begin{graybox}[{Uso del comando \texttt{DSolve}}]
...
La sintaxis que se debe usar para obtener una representación del campo vectorial ${\bf F}(x,y,z)$ es \textls[-10]{\verb"VectorPlot3D[F[x,y,z],{x,a,b},{y,c,d},{z,p,q}]"}.\index{VectorPlot3D}

El campo vectorial ${\bf F}(x,y)= \la \frac{-x-y}{\sqrt{x^2+y^2}}, \frac{x-y}{\sqrt{x^2+y^2}} \ra$ se representa para $x \in [-1,1]$, $y \in [-1,1]$. También se presentan algunas \emph{líneas de flujo}\footnote{Para un campo vectorial ${\bf F}$, una línea de flujo para ${\bf F}$ es una curva ${\bf r}(t)$ tal que ${\bf r}^\prime(t) = {\bf F}({\bf r}(t))$. Esto es, ${\bf F}$ corresponde al campo de velocidades de la curva ${\bf r}(t)$.}, esto se logra
...
\includegraphics[height=5.5cm]{Img/T_55a.pdf} \hspace{0.5cm}
\includegraphics[height=5.5cm]{Img/T_55b.pdf}
\end{center}
\fuentepropia
\end{figure}
...
\begin{tcolorbox}[title=Instrucciones en Mathematica]
\begin{Verbatim}[formatcom=\letraverbatim]
VectorPlot[{-(x+y)/Sqrt[x^2+y^2],(x-y)/Sqrt[x^2+y^2]},
{x,-1,1},{y,-1,1},VectorStyle->Black]
StreamPlot[{-(x+y)/Sqrt[x^2+y^2],(x-y)/Sqrt[x^2+y^2]},
{x,-1,1},{y,-1,1},StreamStyle->Black,StreamScale->0.08]
\end{Verbatim}
\end{tcolorbox}
...
\begin{tcolorbox}[title=Campos vectoriales sobre superficies]
\begin{Verbatim}[formatcom=\letraverbatim]
...
\begin{tcolorbox}[title=Instrucciones en Mathematica]
\begin{Verbatim}[formatcom=\letraverbatim]
SliceVectorPlot3D[{y,-x,x+z},{(x^2+y^2)^2==z+1,z==2*(1-x^2-y^2)},
{x,-1,1},{y,-1,1},{z,-1,2.2},
RegionFunction->Function[{x,y,z},x^2+y^2<=1],
VectorScale->{0.18,Scaled[0.3],Automatic},
VectorStyle->Black,VectorPoints->16,PlotStyle->Orange]
\end{Verbatim}
\end{tcolorbox}

\begin{figure}[H]
\centering
\caption{Campo vectorial ${\bf F}(x,y,z)= \la y,-x,x+z \ra$}
\includegraphics[height=6cm]{Img/T_28a.pdf} \hspace{5mm}
\includegraphics[height=6cm]{Img/T_28b.pdf}
\fuentepropia
\end{figure}
...
\begin{figure}[H]
\centering
\caption{Campo vectorial ${\bf F}(x,y,z)= \la -y,x,z/2 \ra$}
\includegraphics[height=6cm]{Img/T_27a.pdf} \hspace{5mm}
\includegraphics[height=6cm]{Img/T_27b.pdf}
\fuentepropia
\end{figure}
...
\begin{tcolorbox}[title=Instrucciones en Mathematica]
\begin{Verbatim}[formatcom=\letraverbatim]
SliceVectorPlot3D[{-y,x,z/2},{4*z==4-x^2-y^2},{x,-2,2},
{y,-2,2},{z,0,1.1},RegionFunction->Function[{x,y,z},
x^2+y^2<=4],VectorScale->{0.15,Scaled[0.25],Automatic},
VectorStyle->Black,VectorPoints->16,PlotStyle->Orange,
...
ListSliceVectorPlot3D[datos,ImplicitRegion[g[x,y,z]==0,{x,y,z}],
...
\begin{figure}[H]
\centering
\caption{Campo vectorial ${\bf F}(x,y,z) =\la x+z,y,2x-z \ra$}
\includegraphics[height=4.5cm]{Img/T_33a.pdf} \hspace{5mm}
\includegraphics[height=4.5cm]{Img/T_33b.pdf}
\fuentepropia
\end{figure}
...
\begin{tcolorbox}[title=Instrucciones en Mathematica]
\begin{Verbatim}[formatcom=\letraverbatim]
datos=Table[{x+z,y,2*x-z},{x,-2,2},{y,-2,2},{z,-1,1}];
...
\begin{figure}[H]
\centering
\caption{Campo vectorial ${\bf F}(x,y,z) =\la x-z,y,z+2 \ra$}
\includegraphics[height=4.5cm]{Img/T_54a.pdf} \hspace{5mm}
\includegraphics[height=4.5cm]{Img/T_54b.pdf}
\fuentepropia
\end{figure}
...
%\clearpage
\begin{ejemplo}
\textls[-20]{El campo vectorial {\small ${\bf F}(x,y,z) = \left\langle y^{3}e^{-z},3xy^{2}e^{-z},3z^{2}-xy^{3}e^{-z}\right\rangle$}}
