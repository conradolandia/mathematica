\chapter{Superficies y sus curvas de intersección}\label{superficiesysuscurvas}
...
\textls[-20]{De acuerdo con \cite{marsden_calculo_2013}, la longitud de arco $L$ de la curva $C$ en $\mathbb{R}^3$, determinada por la función vectorial $\overrightarrow{r}(t)=\left\langle x(t) ,y(t),z(t) \right\rangle$ para $t\in \lbrack a,b]$, está dada por:}
...
\begin{center}
  ...
  \includegraphics[height=4cm]{Img/72a.pdf} \hskip0.5cm
  \includegraphics[height=4cm]{Img/72b.pdf}
  \fuentepropia
\end{center}
\end{figure}
...
a=ContourPlot3D[{2*z==x^2+y^2,-x+y+z==1},{x,-5,4},{y,-4,5},{z,0,6},
ContourStyle->{Orange,Cyan},PlotPoints->60,Mesh->False,
Ticks->{{-3,0,3},{-3,0,3},{1,2,4,5}}];
b=ParametricPlot3D[{1+2*Cos[t],-1+2*Sin[t],3+2*Cos[t]-2*Sin[t]},
...
\]
Por lo tanto, la función vectorial
...
\label{fig:116}
\includegraphics[height=5cm]{Img/70a.pdf} \hspace{5mm}
\includegraphics[height=5cm]{Img/70b.pdf}
\fuentepropia
\end{figure}

Las instrucciones para conseguir la figura~\ref{fig:116} son las siguientes:

\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]
\begin{Verbatim}[formatcom=\letraverbatim]
a=ContourPlot3D[{x+3==(y-2)^2+(z-1)^2,(z-2)^2+(y-2)^2==4},{x,-3,7},
{y,-2,6},{z,-2.2,4.5},Mesh->False,
ContourStyle->{Yellow,RGBColor[0.8,0.3,0.2]},PlotPoints->60];
b=ParametricPlot3D[{2+4*Sin[t],2+2*Cos[t],2+2*Sin[t]},
{t,0,2*Pi},PlotStyle->{Black,Thickness->0.014},PlotPoints->80];
Show[a,b]
\end{Verbatim}
\end{tcolorbox}
...
\]
\textls[-15]{Este cálculo se puede obtener numéricamente con las siguientes sentencias:}
...
$z=x^{2}-y^{2}$ se intersecan en una curva como lo muestra la figura~\ref{fig:67}.

\begin{figure}[H]
\centering
\caption{Gráfico de $x^{2}+y^{2}=4$ y $z=x^{2}-y^{2}$}
\label{fig:67}
\includegraphics[height=4.5cm]{Img/67a.pdf} \hspace{5mm}
\includegraphics[height=4.5cm]{Fig/301.pdf}
\fuentepropia
\end{figure}
...
la curva de intersección de las superficies señaladas.
...
\begin{gather*}
\left\| \overrightarrow{r}^{\prime }(t) \right\| = 2\sqrt{64\cos ^{2}(t) +1-64\cos ^{4}(t)}= 2\sqrt{9-8\cos \left( 4t\right)}.
\end{gather*}
Con lo cual, la longitud de esta curva está dada por:
\begin{gather*}
\int_{0}^{2\pi }2\sqrt{9-8\cos \left( 4t\right) }dt \approx 35.2571.
\end{gather*}
\textls[-20]{La figura y la evaluación numérica de esta integral se obtienen en
\verb"Mathematica"} mediante el uso se las siguientes instrucciones:

\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]
\begin{Verbatim}[formatcom=\letraverbatim]
r={2*Cos[t],2*Sin[t],4*Cos[2*t];
Show[ContourPlot3D[{x^2+y^2==4,z==x^2-y^2},{x,-4,4},{y,-4,4},
{z,-5,5},Mesh->False,PlotPoints->50,
...
\includegraphics[height=4.5cm]{Img/50a.pdf} \hspace{5mm}
\includegraphics[height=4.5cm]{Img/50b.pdf}
\fuentepropia
\end{figure}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]
\begin{Verbatim}[formatcom=\letraverbatim]
A=ContourPlot3D[{z==5-y^2,z==x^2+y^2},{x,-3,3},{y,-3,3},{z,0,5},
PlotPoints->90,ContourStyle->{Cyan,Orange},Mesh->False]
r={Sqrt[5]*Cos[t],Sqrt[5/2]*Sin[t],(5/2)*(1+Cos[t]^2)};
B=ParametricPlot3D[r,{t,0,2Pi},PlotStyle->{Black,Thickness->0.012}];
Show[A,B]
\end{Verbatim}
\end{tcolorbox}
...
\includegraphics[height=4cm]{Img/53a.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Img/53e.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Img/53f.pdf}
\fuentepropia
\end{figure}
...
\textls[-15]{Con estas expresiones se puede reemplazar el valor de $x$ para determinar su proyección en el plano $yz$.
...
\includegraphics[height=4cm]{Img/71c.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Img/71a.pdf}
\fuentepropia
\end{figure}
...
\begin{gather*}
\int_{0}^{2\pi }\frac{1}{16}\sqrt{512-55\cos ^{2}(t) -169\cos^{4}(t) }dt \approx 8.019.
\end{gather*}
...
\begin{Verbatim}[formatcom=\letraverbatim]
r={Sqrt[2]*Cos[t],9/32-13/32*Cos[t]^2,3*Sqrt[2]/4*Sin[t]};
A=Assuming[t>=0,Simplify[Norm[D[[r,t]]]]
NIntegrate[A,{t,0,2*Pi}]
\end{Verbatim}
\end{tcolorbox}
...
\includegraphics[height=5.5cm]{Img/1f.pdf} \hspace{5mm}
\includegraphics[height=5.5cm]{Fig/78.pdf}
\fuentepropia
\end{figure}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]
\begin{Verbatim}[formatcom=\letraverbatim]
ContourPlot3D[{z==6-(x-1)^2-y^2,4*x^2+y^2==36,x^2+4*y^2==4},
{x,-6,10},{y,-7,7},{z,-35,6},
...
\includegraphics[height=4.5cm]{Img/87a.pdf} \hspace{5mm}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]
\begin{Verbatim}[formatcom=\letraverbatim]
z=Sqrt[2]*Cosh[t],Sqrt[2]*Cosh[t];x=3*Sqrt[2]*Sinh[t];
y=Sqrt[2]*Sqrt[6+Cosh[t]^2-18*Sinh[t]^2];
...
\includegraphics[height=5cm]{Img/6a.pdf} \hspace{5mm}
\includegraphics[height=5cm]{Fig/79.pdf}
\fuentepropia
\end{figure}
...
\[{\bf r}_1(t) =\left\langle 1+\cos (t) ,\sin (t),\sqrt{2-2\cos (t) }\right\rangle ,\,\, t\in [ 0,2\pi ],\]

parametriza la curva de intersección. Es de anotar que son dos porciones que se obtienen por la raíz positiva y la raíz negativa.

Para la expresión  $x^{2}+z^{2}=2x$ (que representa el otro cilindro), el tratamiento es similar, obteniéndose la función
...
\includegraphics[height=4cm]{Img/58a.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Img/58b.pdf}
\fuentepropia
\end{figure}

Sobre el paraboloide se cumple que
$z=\frac{9}{2}-2 \sin (t)+\sin (2 t)-2 \cos (t)+\frac{3}{2} \cos (2 t)$.
Por lo tanto, para $t\in [0,2\pi]$, la curva de intersección del paraboloide y el cilindro cardióidico se parametriza por medio de la siguiente función:
\begin{multline*}
...
{y,-4,2},{z,0,3},ContourStyle->{Orange},Mesh->False,PlotPoints->80];
...
Entonces, $x-z=2t;$ por lo tanto, $\left( x-z\right) ^{2}=4t^{2}=\frac{4}{3}\left(
3t^{2}\right) =\frac{4}{3}y$. Además $x+z=6t^{3}=\left( 2t\right) \left( 3t^{2}\right) =\left(
x-z\right) y$.

Por tal razón, utilizaremos las expresiones $x-z=\frac{4}{3}y$ y $x+z=y\left( x-z\right) $,
cuya figura se presenta a continuación y con esta también se muestra la curva de intersección.
...
b=ContourPlot3D[{(x-z)^2==4*y/3,x+z==y*(x-z)},{x,-1,1},{y,-0.5,1},
{z,-1,1},Mesh->False,ContourStyle->{Directive[RGBColor[0.3,0.6,0.3],
...
\includegraphics[height=4cm]{Img/94a.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Img/94b.pdf}
\fuentepropia
\end{figure}
...
\includegraphics[height=4cm]{Img/92a.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Img/92b.pdf}
\fuentepropia
\end{figure}
...
\includegraphics[height=4cm]{Img/93a.pdf} \hskip0.5cm
\includegraphics[height=4cm]{Img/93b.pdf}
\fuentepropia
\end{center}
\end{figure}
...
Los gráficos de las dos superficies junto con la curva de intersección se muestran en la figura~\ref{fig:128}.

\begin{figure}[H]
\centering
\caption{Superficies con intersección ${\bf r}(t)=\la 3t-t^{3},3t^2,3t+t^{3} \ra$}
\label{fig:128}
\includegraphics[height=4cm]{Img/91a.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Img/91b.pdf}
\fuentepropia
\end{figure}
...
\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
a=ParametricPlot3D[{3*t-t^3,3*t^2,3*t+t^3},{t,-1.15,1.15},
PlotStyle->{Black,Thickness->0.017},PlotPoints->90];
b=ContourPlot3D[{x^2-z^2+4*y^2/3,(x+z)^2==12*y},{x,-5,5},
{y,-5,5},{z,-5,5},ContourStyle->{Directive[Orange,Opacity[0.7]],
Directive[RGBColor[0.5,0.8,0.5],Opacity[0.6]]}],Mesh->False];
Show[b,a]
\end{Verbatim}
\end{tcolorbox}
...
A partir de las funciones componentes y de algunas identidades trigono\-métricas conocidas se pueden establecer relaciones entre ellas;
dos de éstas funciones son: $x^2=1+z^2$ y $y=\ln(x+z)$.
Los gráficos de estas dos superficies junto con la curva de intersección se muestran en la figura~\ref{fig:129aa}.

\begin{figure}[t]
\centering
\caption{Superficies con intersección ${\bf r}(t)=\la \sec t, \ln (\sec(t)+\tan(t)),\tan(t) \ra$}
\label{fig:129aa}
\includegraphics[height=4cm]{Img/95a.pdf} \hspace{5mm}
\includegraphics[height=4cm]{Img/95b.pdf}
\fuentepropia
\end{figure}

Las instrucciones para conseguir esta figura son las siguientes:

\begin{tcolorbox}[title=\bf Instrucción \verb"Mathematica"]\begin{Verbatim}[formatcom=\letraverbatim]
a=ParametricPlot3D[{Sec[t],Log[Sec[t]+Tan[t]],Tan[t]},
{t,-Pi/3,Pi/3},PlotStyle->{Thickness[0.017],Black}];
b=ContourPlot3D[{1+z^2==x^2,y==Log[x+z]},{x,-2,3},{y,-2,2},{z,-2,2},
Mesh->False,ContourStyle->{Directive[Orange,Opacity[0.95]],
...
\includegraphics[height=4.5cm]{Img/T_23a.pdf}\hskip0.3cm
...
\includegraphics[height=4.5cm]{Img/T_24a.pdf}\hskip0.3cm
...
\includegraphics[height=4.5cm]{Img/T_25a.pdf}\hskip0.3cm
\includegraphics[height=4.5cm]{Img/T_25b.pdf}
\fuentepropia
\end{center}
\end{figure}

\item \textls[-20]{El cilindro cardióidico con ecuación $r=1+\cos(t)$ y el plano $x+y+3z=1$.}\index{Cilindro cardiodico}

\begin{figure}[H]
\begin{center}
\caption{Gráfico de $r=1+\cos(t)$ y $x+y+3z=1$}
\includegraphics[height=4.5cm]{Img/T_29a.pdf}\hskip0.3cm
...
\includegraphics[height=3.5cm]{Img/T_72a.pdf}\hskip0.3cm
\includegraphics[height=3.5cm]{Img/T_72b.pdf}
\fuentepropia
\end{center}
\end{figure}
...
\includegraphics[height=3.5cm]{Img/97a.pdf}\hskip0.3cm
\includegraphics[height=3.5cm]{Img/97b.pdf}
\fuentepropia
\end{center}
\end{figure}
...
\includegraphics[height=3.5cm]{Img/98a.pdf}\hskip0.3cm
\includegraphics[height=3.5cm]{Img/98b.pdf}
\fuentepropia
\end{center}
\end{figure}
\end{enumerate}
